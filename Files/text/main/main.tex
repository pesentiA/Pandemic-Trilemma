...
$F^{CP}_{i,k-2} \times y_{i,k-1}$ \; [$-\eta_p$]
  & $-0.024$*** & $-0.005$    & $-0.011$*** \\
...
\end{tabular}
\vspace{4pt}
\parbox{\textwidth}{\scriptsize\textit{Notes:} Dependent variable: output gap $y_{ik}$ (pp of potential GDP). 38 OECD countries, Q1\,2020--Q2\,2022 (10~quarters). All columns include the identical set of regressors; columns differ only in the estimator. Standard errors clustered by country in parentheses. Column~(1): pooled OLS. Column~(2): country fixed effects. Column~(3): two-way fixed effects (country + quarter). The OLS estimate of $\eta_p$ ($-0.024$) is inflated by cross-country confounding; country FE absorb this and the coefficient collapses to $-0.005$ (insignificant). Quarter FE restore identification from within-country variation across quarters, yielding $\eta_p = -0.011$ ($p < 0.001$). The lockdown amplification parameter $\psi$ follows a parallel pattern: negative in OLS (sign reversal from omitted-variable bias), zero under country FE, and correctly signed and significant under TWFE. The level of $F^{CP}$ is omitted because the theoretical framework predicts no contemporaneous level effect; a specification including $F^{CP}$ as a level control is reported in Section~\ref{subsec:robustness}. * $p<0.1$, ** $p<0.05$, *** $p<0.01$.}
\end{table}

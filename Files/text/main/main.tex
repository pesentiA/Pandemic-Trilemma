...
to other crisis types. Section~\ref{sec:planner} develops the iLQR optimization
framework. Section~\ref{sec:estimation} describes the micro-level fiscal policy database.
Section~\ref{sec:estimation} presents the estimation strategy and structural parameter
estimates. Section~\ref{sec:results_empirical} reports the optimal composition trajectories and
...
Figure~\ref{fig:planing_horizon} illustrates with the OECD average the 
...
\begin{table}[H]
\centering
\caption{Main Results: Output Gap Equation (V14, 12.05.2026)}
...
\toprule
 ...
 &            & (Main)     & (Robustness) \\
\midrule
\multicolumn{4}{@{}l}{\textit{Persistence channel:}} \\[2pt]
$y_{i,k-1}$ \,($\rho_y$)
  & $0.566$***  & $0.231$***  & $0.172$*** \\
  & (0.085)     & (0.050)     & (0.035) \\[4pt]
\multicolumn{4}{@{}l}{\textit{Containment channel:}} \\[2pt]
$S_{ik}$ \,($\alpha_S$)
  & $-0.065$*** & $-0.095$*** & $-0.104$*** \\
  & (0.012)     & (0.013)     & (0.013) \\[4pt]
\multicolumn{4}{@{}l}{\textit{Capacity-preservation channels:}} \\[2pt]
$F^{CP,\,\text{above}}_{i,k-2}$ \,(flow) \,[$\alpha_{\text{above}}$]
  & $1.024$***  & $0.544$***  & $0.728$*** \\
  & (0.264)     & (0.189)     & (0.182) \\[4pt]
$K^{CP,\,\text{below}}_{ik}$ \,(stock) \,[$\alpha_{\text{below}}$]
  & $-0.037$    & $0.261$*    & $-0.361$** \\
  & (0.100)     & (0.145)     & (0.143) \\[4pt]
\multicolumn{4}{@{}l}{\textit{Demand-injection channels (state-dependent):}} \\[2pt]
$F^{DI}_{i,k-1}$ \,($\alpha_{DI}$)
  & $1.552$*    & $1.470$**   & $1.443$*** \\
  & (0.878)     & (0.569)     & (0.521) \\[4pt]
$F^{DI}_{i,k-1} \times S_{ik}$ \,($\alpha_{S\cdot DI}$)
  & $-0.036$*   & $-0.041$*** & $-0.035$*** \\
  & (0.019)     & (0.014)     & (0.011) \\[4pt]
\midrule
Country FE       &            & \checkmark & \checkmark \\
Year FE          &            &            & \checkmark \\
Observations     & 418        & 418        & 418 \\
$R^2$ (adj.)     & 0.461      & 0.590      & 0.690 \\
Within $R^2$     &            & 0.436      & 0.358 \\
\bottomrule
\end{tabular}
\vspace{4pt}
\parbox{\textwidth}{\scriptsize\textit{Notes:} Dependent variable: output gap $y_{ik}$ (percentage points of potential GDP). Sample: 38 OECD countries over Q4\,2019--Q2\,2022 (11 quarters per country, $t \in [4,14]$, $N = 418$). All columns share the V14 regressor set: persistence ($y_{i,k-1}$), containment ($S_{ik}$), capacity-preservation Flow (above-the-line wage subsidies and direct grants, entering with a two-quarter authorisation-to-spending lag), capacity-preservation Stock (cumulative below-the-line loans and guarantees at the low-take-up baseline, 40\% loans / 25\% guarantees), and a state-dependent demand-injection block ($F^{DI}_{i,k-1}$ entered both as a level and interacted with $S_{ik}$). Standard errors clustered at the country level (CRV1, small-sample adjusted) in parentheses. Column~(2) is the preferred specification: Country fixed effects absorb time-invariant structural heterogeneity (fiscal space, institutions, voluntary response patterns) while leaving the global pandemic shock as part of the policy environment to be evaluated, which Quarter or Year fixed effects would absorb mechanically. Column~(3) adds Year fixed effects as a robustness check; the sign reversal of $\alpha_{\text{below}}$ reflects the mechanical absorption of the cumulating below-the-line stock by Year FE rather than identification failure. The state-dependent DI multiplier breaks even at $S^{*} \approx |\alpha_{DI} / \alpha_{S\cdot DI}| \approx 36$, consistent with a push-on-string interpretation: positive baseline multiplier, contractionary at high containment. * $p<0.1$, ** $p<0.05$, *** $p<0.01$.}
\end{table}
...
Table~\ref{tab:variance_decomp} decomposes the variance of each 
regressor into between-country, within-country, and residual 
components after two-way fixed effects absorption. Two patterns 
are central to identification.

\begin{table}[H]
\centering
\caption{Variance Decomposition of Regressors}
\label{tab:variance_decomp}
\scriptsize
\renewcommand{\arraystretch}{1.15}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l r r r r @{}}
\toprule
 & Total SD & Between SD & Within SD & $R^2$ absorbed (C+Q FE) \\
\midrule
Output gap, $y_{ik}$                   
  & $4.77$ & $3.05$ & $3.66$ & $0.843$ \\[2pt]
Output gap, $y_{i,k-1}$                
  & $4.72$ & $2.79$ & $3.81$ & $0.827$ \\[2pt]
Stringency, $S_{ik}$                   
  & $18.70$ & $6.22$ & $17.63$ & $0.756$ \\[2pt]
Capacity preservation, $F^{CP}_{i,k-2}$
  & $3.03$ & $0.70$ & $2.95$ & $0.448$ \\[2pt]
Demand injection, $F^{DI}_{i,k-2}$     
  & $0.71$ & $0.25$ & $0.66$ & $0.246$ \\[2pt]
Excess mortality, $d_{ik}$             
  & $14.87$ & $7.70$ & $12.73$ & $0.433$ \\[2pt]
$S_{ik} \cdot y_{i,k-1}$               
  & $240.77$ & $122.80$ & $207.10$ & $0.725$ \\[2pt]
$F^{CP}_{i,k-2} \cdot y_{i,k-1}$       
  & $38.14$ & $10.39$ & $36.70$ & $0.428$ \\
\bottomrule
\end{tabular*}

\vspace{3pt}
\parbox{\textwidth}{\scriptsize\textit{Notes:} Estimation sample 
(Q1\,2020--Q2\,2022, $N=380$). $R^2$ absorbed reports the share of 
raw variance explained by country and quarter fixed effects jointly.}
\end{table}

\noindent First, containment and the output gap are strongly 
synchronized across countries---fixed effects absorb 76\% and 84\% 
of their variance, respectively---reflecting the common 
epidemiological cycle. Fiscal deployment, by contrast, is 
substantially less synchronized: 55\% of the variance in 
$F^{CP}_{k-2}$ and 75\% in $F^{DI}_{k-2}$ remains as identifying 
variation after absorption of fixed effects. This asymmetry 
reflects the institutional separation between health authorities, 
which coordinated containment along the epidemiological cycle, 
and finance ministries, which deployed fiscal instruments along 
national administrative timetables.

Second, Table~\ref{tab:residual_corr} reports the correlation 
matrix of the fixed-effects residuals. The correlation between 
containment and fiscal instruments is effectively zero 
($-0.014$ with $F^{CP}_{k-2}$; $-0.020$ with $F^{DI}_{k-2}$), and 
the two fiscal instruments are only weakly correlated with each 
other ($0.082$). The non-negligible correlation between 
containment and excess mortality ($0.182$) reflects the well-known 
feedback from observed lethality to policy stringency; conditional 
on both $S_{ik}$ and $d_{ik}$, the fiscal coefficients are 
identified from variation in instrument composition orthogonal 
to the severity of the crisis.

\begin{table}[H]
\centering
\caption{Residual Correlations after Country and Quarter FE}
\label{tab:residual_corr}
\scriptsize
\renewcommand{\arraystretch}{1.15}
\begin{tabular*}{0.7\textwidth}{@{\extracolsep{\fill}} l r r r r @{}}
\toprule
 & $S_{ik}$ & $F^{CP}_{i,k-2}$ & $F^{DI}_{i,k-2}$ & $d_{ik}$ \\
\midrule
$S_{ik}$          & $1.000$  & $-0.014$ & $-0.020$ & $0.182$ \\
$F^{CP}_{i,k-2}$  & $-0.014$ & $1.000$  & $0.082$  & $0.015$ \\
$F^{DI}_{i,k-2}$  & $-0.020$ & $0.082$  & $1.000$  & $-0.064$ \\
$d_{ik}$          & $0.182$  & $0.015$  & $-0.064$ & $1.000$ \\
\bottomrule
\end{tabular*}

\vspace{3pt}
\parbox{0.7\textwidth}{\scriptsize\textit{Notes:} Correlations of 
residuals after two-way fixed effects demeaning. Estimation sample 
(Q1\,2020--Q2\,2022, $N=380$).}
\end{table}

\begin{table}[H]
\centering
\caption{Main Results: Debt Equation (V15, 22.05.2026)}
\label{tab:debt_main}
\scriptsize
\renewcommand{\arraystretch}{1.2}
...
\toprule
 & (1) & (2) & (3) & (4) & (5) \\
 & Pooled OLS & \multicolumn{2}{c}{Country FE --- raw $\Delta b_{ik}$}
              & \multicolumn{2}{c}{Country FE --- adjusted $\Delta b^{\text{adj}}_{ik}$} \\
 \cmidrule(lr){3-4}\cmidrule(lr){5-6}
 & (adj.\ LHS) & Three-way & Pooled below & Main (V15) & + Linear trend \\
\midrule
$y_{ik}$ \,[$-\gamma_y$]
  & $-0.137$*** & $-0.189$*** & $-0.191$*** & $-0.200$*** & $-0.181$*** \\
  & (0.025)     & (0.032)     & (0.031)     & (0.031)     & (0.033) \\[4pt]
$F^{CP,\,\text{above}}_{ik}$ \,[$\kappa^{\text{above}}$]
  & $0.574$***  & $0.488$***  & $0.461$**   & $0.534$***  & $0.394$** \\
  & (0.186)     & (0.178)     & (0.178)     & (0.173)     & (0.194) \\[4pt]
$F^{CP,\,\text{loans}}_{ik}$ \,(40\% take-up) \,[$\kappa^{\text{loans}}$]
  & $0.930$***  & $0.935$***  &             & $0.957$***  & $0.893$*** \\
  & (0.210)     & (0.185)     &             & (0.201)     & (0.170) \\[4pt]
$F^{CP,\,\text{guar}}_{ik}$ \,(25\% take-up) \,[$\kappa^{\text{guar}}$]
  & $0.196$     & $0.189$     &             & $0.197$     & $0.083$ \\
  & (0.246)     & (0.230)     &             & (0.246)     & (0.227) \\[4pt]
$F^{CP,\,\text{below}}_{ik}$ \,(pooled) \,[$\kappa^{\text{below}}$]
  &             &             & $0.497$***  &             &  \\
  &             &             & (0.142)     &             &  \\[4pt]
$F^{DI}_{i,k-1}$ \,[$\kappa^{DI}$]
  & $0.580$*    & $0.449$     & $0.467$*    & $0.492$*    & $0.414$ \\
  & (0.306)     & (0.269)     & (0.267)     & (0.270)     & (0.258) \\[4pt]
Linear trend ($t$)
  &             &             &             &             & $-0.110$*** \\
  &             &             &             &             & (0.025) \\[4pt]
\midrule
Country FE              &            & \checkmark & \checkmark & \checkmark & \checkmark \\
Interest-rate adj.\ LHS & \checkmark & ---        & ---        & \checkmark & \checkmark \\
Observations            & 494        & 494        & 494        & 494        & 494 \\
$R^2$ (adj.)            & 0.220      & 0.199      & 0.195      & 0.214      & 0.237 \\
Within $R^2$            &            & 0.243      & 0.238      & 0.259      & 0.282 \\
\bottomrule
\end{tabular}
\vspace{4pt}

\parbox{\textwidth}{\scriptsize\textit{Notes:} Dependent variable: quarterly change in real public debt $\Delta b_{ik}$ (pp of 2019 GDP). Columns~(2)--(3) use the raw change $\Delta b_{ik}$; columns~(1), (4) and~(5) use the interest-rate-adjusted change $\Delta b^{\text{adj}}_{ik} = \Delta b_{ik} - r^{\text{LT}}_k \cdot b_{i,k-1}/4$, where $r^{\text{LT}}_k$ is the OECD cross-country median long-term nominal interest rate at quarter $k$. The adjusted LHS nets out mechanical interest accrual so that the residual $\Delta b^{\text{adj}}$ captures the primary balance and discretionary financing operations. Sample: 38 OECD countries over Q4\,2019--Q4\,2022 (13 quarters per country, $t \in [4,16]$, $N = 494$). Capacity preservation is decomposed three-way: above-the-line outlays (wage subsidies, direct grants), below-the-line loans (40\% drawdown, ECB SAFE low scenario), and below-the-line guarantees (25\% conversion). Column~(3) reports the pooled-below variant ($F^{CP,\,\text{loans}} + F^{CP,\,\text{guar}}$) for comparison; columns~(2), (4) and~(5) use the three-way decomposition. Column~(4) is the V15 main specification (22.05.2026); column~(5) adds a linear time trend as robustness. Standard errors clustered by country (CRV1, small-sample adjusted) in parentheses. Loans deliver the largest realised debt impact ($\hat{\kappa}^{\text{loans}} \approx 0.94$), above-the-line is the second-largest channel, while guarantees translate into negligible realised debt over the sample period---consistent with the ``whatever-it-takes'' interpretation that contingent liabilities preserve productive capacity through announcement effects without being drawn. * $p<0.1$, ** $p<0.05$, *** $p<0.01$.}
\end{table}

\subsection{Identifying Variation in the Debt Equation}

Table~\ref{tab:variance_decomp_debt} reports the analogous 
variance decomposition for the debt equation. Because the debt 
equation does not include quarter fixed effects---the output gap 
$y_{ik}$ absorbs common temporal dynamics through the automatic 
stabilizer channel---the relevant benchmark is within-country, 
across-quarter variation.

\begin{table}[H]
\centering
\caption{Variance Decomposition of Regressors, Debt Equation}
\label{tab:variance_decomp_debt}
\scriptsize
\renewcommand{\arraystretch}{1.15}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l r r r r @{}}
\toprule
 & Total SD & Between SD & Within SD & $R^2$ absorbed (C FE) \\
\midrule
Change in debt, $\Delta b_{ik}$              
  & $2.48$ & $0.54$ & $2.42$ & $0.047$ \\[2pt]
Output gap, $y_{ik}$                         
  & $4.77$ & $3.05$ & $3.66$ & $0.401$ \\[2pt]
CP above-the-line, $F^{CP,\text{above}}_{ik}$
  & $0.98$ & $0.27$ & $0.94$ & $0.076$ \\[2pt]
CP loans, $F^{CP,\text{loans}}_{ik}$         
  & $1.02$ & $0.30$ & $0.97$ & $0.084$ \\[2pt]
CP guarantees (adj.), $F^{CP,\text{guar}}_{ik}$
  & $0.73$ & $0.18$ & $0.71$ & $0.058$ \\[2pt]
Demand injection, $F^{DI}_{ik}$              
  & $0.71$ & $0.25$ & $0.66$ & $0.127$ \\[2pt]
Health, $F^{H}_{ik}$                         
  & $0.38$ & $0.12$ & $0.36$ & $0.104$ \\
\bottomrule
\end{tabular*}

\vspace{3pt}
\parbox{\textwidth}{\scriptsize\textit{Notes:} Estimation sample 
(Q1\,2020--Q2\,2022, $N = 380$). $R^2$ absorbed reports the share 
of raw variance explained by country fixed effects alone.}
\end{table}

\noindent Country fixed effects absorb only a small share of the 
variance in pandemic-era fiscal variables---between 6\% and 13\%---
because pandemic-specific fiscal instruments had no pre-pandemic 
counterpart and cross-country level differences are therefore 
limited. More than 85\% of the variance in each fiscal component 
is identifying.

Table~\ref{tab:residual_corr_debt} reports residual correlations 
after country fixed effects. Two features merit discussion. 
First, the fiscal components are moderately correlated with one 
another (0.15--0.33), reflecting a common pattern of fiscal 
generosity: countries that deployed large volumes in one channel 
also deployed in others. This correlation is a source of 
multicollinearity that enlarges standard errors on the individual 
$\kappa$ coefficients, especially for guarantees, whose residual 
correlation with above-the-line (0.333) and loans (0.301) is the 
highest in the matrix. Second, the output gap is negatively 
correlated with all fiscal components ($-0.20$ to $-0.23$), 
consistent with deeper recessions eliciting larger fiscal 
responses. The $\gamma_y$ coefficient is therefore identified from 
residual variation in $y_{ik}$ net of this common countercyclical 
deployment pattern.

\begin{table}[H]
\centering
\caption{Residual Correlations after Country FE, Debt Equation}
\label{tab:residual_corr_debt}
\scriptsize
\renewcommand{\arraystretch}{1.15}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l r r r r r r @{}}
\toprule
 & $y$ & above & loans & guar & $F^{DI}$ & $F^{H}$ \\
\midrule
$y_{ik}$                 
  & $1.000$ & $-0.218$ & $-0.227$ & $-0.215$ & $-0.198$ & $-0.197$ \\
$F^{CP,\text{above}}$    
  & $-0.218$ & $1.000$ & $0.161$ & $0.333$ & $0.258$ & $0.207$ \\
$F^{CP,\text{loans}}$    
  & $-0.227$ & $0.161$ & $1.000$ & $0.301$ & $0.219$ & $0.205$ \\
$F^{CP,\text{guar}}$     
  & $-0.215$ & $0.333$ & $0.301$ & $1.000$ & $0.161$ & $0.152$ \\
$F^{DI}$                 
  & $-0.198$ & $0.258$ & $0.219$ & $0.161$ & $1.000$ & $0.199$ \\
$F^{H}$                  
  & $-0.197$ & $0.207$ & $0.205$ & $0.152$ & $0.199$ & $1.000$ \\
\bottomrule
\end{tabular*}

\vspace{3pt}
\parbox{\textwidth}{\scriptsize\textit{Notes:} Correlations of 
residuals after country fixed effects demeaning. Estimation sample 
(Q1\,2020--Q2\,2022, $N = 380$).}
\end{table}
%5NEXT STEPS
...
\end{tabular}
\vspace{4pt}
\parbox{\textwidth}{\scriptsize\textit{Notes:} Output parameters from the V14 Country-FE specification (Table~\ref{tab:main_results}, column 2). Debt parameters from the V15 main specification with interest-rate-adjusted LHS and three-way CP decomposition (Table~\ref{tab:debt_main}, column 4). Standard errors clustered at the country level in parentheses. * $p<0.1$, ** $p<0.05$, *** $p<0.01$.}
\end{table}

...
\addlinespace
\multicolumn{3}{@{}l}{\textit{F.\ Heterogeneity (sample split)}}\\
Social safety net, high vs.\ low ($19+19$ countries) & Whether channel pass-through differs by welfare-state strength (same groupings as the output equation) & Loans concentrate in \emph{low}-soc.net ($\kappa_{\text{loans}}=2.61^{**}$ vs.\ $0.45$ n.s.); demand injection adds to debt only in \emph{high}-soc.net ($\kappa_{DI}=1.23^{**}$ vs.\ $0.12$ n.s.); the stabiliser is stronger in high-soc.net ($\gamma_y=-0.16^{**}$ vs.\ $-0.09$). Mirrors the output-gap split. \\
\bottomrule
\end{tabular}
\vspace{4pt}
...
also identifies it. A split by welfare-state strength sharpens the
interpretation: pandemic debt accumulates through below-the-line \emph{loans}
in low-social-net countries but through \emph{demand-injection transfers} in
high-social-net countries---mirroring the heterogeneity documented for the
output equation.

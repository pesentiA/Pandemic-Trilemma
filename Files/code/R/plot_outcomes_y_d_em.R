## ============================================================================
##  Three-panel outcomes plot: Output Gap | Cumulative New Debt | Cumulative
##                              Excess Mortality
...
##  - Panel C: cum_excess_per_million_proj_all_ages
##             = cumulative excess deaths per million (projection baseline)
##  Style:    AER / QJE journal layout
## ============================================================================
...
dat <- qdata %>%
  ...
  mutate(
    ...
    cum_em       = cum_excess_per_million_proj_all_ages
  ) %>%
  ungroup() %>%
  mutate(Country = factor(labels_iso[Country], levels = labels_iso[sel])) %>%
  select(Country, qdate, y_t_pct, cum_new_debt, cum_em)
...
make_panel <- function(df, yvar, ylab, title) {
  ggplot(df, aes(qdate, .data[[yvar]],
    ...
    geom_point(size = 1.2, stroke = 0, na.rm = TRUE) +
...
p_m <- make_panel(dat, "cum_em",
                  ylab  = "Excess deaths per million",
                  title = "Panel C.  Cumulative excess mortality")
...
caption_text <- str_wrap(paste0(
  ...
  "Panel C: cumulative excess deaths per million (pre-pandemic projection baseline)."
), width = 165)

combined <- (p_y + p_d + p_m) +
...
out_file <- file.path(out_dir, "outcomes_y_cumdebt_excess_aer_v2.pdf")

ggsave(out_file, combined,
       width = 13, height = 5.4, units = "in", device = cairo_pdf)

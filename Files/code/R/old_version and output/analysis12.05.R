...
#
# IDENTIFICATION:
#   Country-FE only (NOT Quarter-FE). Rationale: trilemma counterfactuals
#   require the TOTAL F-effect within the realized global pandemic context,
#   not the net-of-trends effect. Quarter-FE would absorb the global shock
#   that is part of the policy environment we want to evaluate. Differences 
#   in Countrys of F based on their S and Voluntary responses
#
...
# Quarter as factor for Year-FE robustness checks
...
#   - Decreasing below stock-> macht ökonomisch keinen Sinn
...
for (d in deltas) {
  df_tmp <- df_bin |>
    group_by(Country) |> arrange(t_idx) |>
    mutate(F_loans_decay = build_decay_stock(F_CP_loans, d),
           F_guar_decay  = build_decay_stock(F_CP_guar,  d),
           F_CP_below_decay = F_loans_decay + F_guar_decay) |> ungroup()
...
##Nicht identifiziert, Dummy wurde gedroppt
#Mit linearem trend
...
pdataD <- pdataD |>
  group_by(Country) |> arrange(t_idx) |>
  mutate(F_CP_below_flow_lag1 = lag(F_CP_below_flow, 1)) |>
  ungroup()
...
# -----------------------------------------------------------------------------
debtFE <- plm(
  debt_dR ~ y_t_pct + F_CP_above_flow +F_CP_below_flow + F_DI_lag1,
  data = pdataD, index = c("Country","Quarter"),
  model = "within", effect = "individual"
)
coeftest(debtFE, vcov = vcovHC(debtFE, cluster = "group", type = "HC1"))
#iLQR dieses verwendet 11.05.2026

#debt_v14_disagg <- feols(debt_dR ~ y_t_pct 
#                         + F_CP_above_3 + F_CP_loans_adj + F_CP_guar_adj
#                         + F_DI_lag1 | Country,
#                         data = pdataD)

...
titles_sub={'Containment (S)','Capacity Preservation (% GDP)','Demand Injection (% GDP)',...
            'Output Gap (pp GDP)','Cumulative Mortality (% pop.)','Public Debt (pp GDP)'};
...
for p=1:6, subplot(2,3,p); hold on;
    switch p
        ...
        case 5  % Cumulative mortality — lines
            for s=1:ns
                plot(0:N, cumsum([0, par.x{si(s)}(2,2:end)*0.0025])*100, '-', 'Color', cols(s,:), 'LineWidth', 1.8);
            end
            oecd_cum_d = [0, cumsum(delta_theta_t .* oecd.theta)*0.0025] * 100;
            plot(0:N, oecd_cum_d, 'k:', 'LineWidth', 2);
            ylabel('% pop.');
        ...
        case 6  % Public debt — lines
            ...
            oecd_cum_b = [0, cumsum(oecd.b)*0.0025] * 100;

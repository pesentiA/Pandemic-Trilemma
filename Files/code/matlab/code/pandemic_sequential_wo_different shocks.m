...
x_unc = zeros(4, N+1);
x_unc(:, 1) = [0; 0; 0; 0.001];  % Pandemic shock only
...
% Weights
w_y = 10;    w_d = 500000000;    w_b = 0.5;    W_b = 10;
...
rng(42);
noise_vec = zeros(6, N);
noise_vec(4,6) = 0.4; %%WEITERE WELLEN ABBILDEN
%noise_vec(4, :) = randn(1, N) * sigma_noise; &&RANDOM

% Vor der Schleife definieren:
delta_theta_vec = 0.01 * ones(1, N); % 1% Sterblichkeit für alle Quartale
delta_theta_vec(8:end) = 0.00005;     % Ab Q8 (Impfung): Sterblichkeit fällt drastisch!
...
%% ========================================================================
%  ITERATIVE LQR (iLQR) mit PROJECTED FEEDFORWARD
% =========================================================================

max_iter  = 100;       
tol       = 1e-7;      

% --- NEU: Zeitvariable Epidemiologie (Impfung ab Q8) ---
% Vor Q8: Exponentielles Wachstum (rho=2.0) und hohe Letalität
% Ab Q8: Impfung senkt Letalität extrem und drückt R-Wert unter 1 (Herdenimmunität)
delta_theta_vec = 0.01 * ones(1, N);
delta_theta_vec(8:end) = 0.0005; % Sterblichkeit sinkt um 95%

rho_theta_vec = 2.0 * ones(1, N);
rho_theta_vec(8:end) = 0.8;      % R-Wert fällt unter 1, Virus stirbt von alleine aus
...
for iter = 1:max_iter
    ...
    p = cell(N+1, 1);   
    K = cell(N, 1);
    k_ff_clamped = cell(N, 1);  % NEU: Der beschränkte Feedforward-Term
    ...
    for k = N:-1:1
        ...
        % Regularisierung falls Quu fast singulär wird
        Quu = Quu + 1e-6 * eye(3);

        K{k} = Quu \ Qux;       
        k_ff = -Quu \ Qu;       

        % NEU: PROJECTED FEEDFORWARD TRICK
        % Wir beschneiden den Zielwert auf wirtschaftlich machbare Maxima
        u_target = u_bar(:, k) + k_ff;
        u_target(1) = max(0, min(1,   u_target(1))); % S in [0, 1]
        u_target(2) = max(0, min(0.2, u_target(2))); % DI max 20% GDP
        u_target(3) = max(0, min(0.2, u_target(3))); % CP max 20% GDP
        
        % Der tatsächliche, machbare Schritt in diese Richtung
    ...
    while alpha > 1e-4
        ...
        for k = 1:N
            % Nutze jetzt den CLAMPED Feedforward!
            u_new(:, k) = u_bar(:, k) + alpha * k_ff_clamped{k} - K{k} * (x_new(:, k) - x_bar(:, k));

            % Constraints erzwingen (zur absoluten Sicherheit)
            u_new(1, k) = max(0, min(1,   u_new(1, k)));    
            u_new(2, k) = max(0, min(0.2, u_new(2, k)));            
            u_new(3, k) = max(0, min(0.2, u_new(3, k)));            

            x_new(:, k+1) = f_nonlin(x_new(:, k), u_new(:, k)); 
        end
...
% Resultate für Plotting sichern (sogar wenn es abbricht, haben wir x_bar)
x_opt = x_bar;
u_opt = u_bar;

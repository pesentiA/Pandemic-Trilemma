...
% --- Output Dynamics ---
rho_y   = 0.9;     % Baseline persistence (Cerra et al., 2023) (0.85-0.95)
psi     = 0.05;     % Hysteresis sensitivity to lockdown (0.02-0.1)

% --- Epidemiology ---
rho_theta   = 3;  % Transmission rate (>1: explosive) (2.5-3)-> spätere Varaianten höher
delta_theta = 0.01;  % Infection fatality rate (0.008-0.010)

% --- Containment (S) ---
alpha_S = 0.12;      % Contractionary effect on output (0.08-0.15)
phi_S   = 0.70;      % Suppression effectiveness on infections (0.6-0.82)

% --- Demand Injection (F_DI) ---
alpha_F_DI = 0.4;   % Fiscal multiplier (0.3-1)->SELBER
kappa_F_DI = 0.4;   % Budgetary cost per unit (0.7-0.9)->SELBER

% --- Capacity Preservation (F_CP) ---
eta        = 0.4;   % Effectiveness on persistence reduction (0.03-0.08)->SELBER
kappa_F_CP = 0.4;   % Budgetary cost per unit (0.5-0.7)->SELBER

% --- Debt Dynamics ---
r       = 0.001;     % Quarterly real interest rate
gamma_y = 0.5;      % Budget semi-elasticity (auto-stabilizers) (0.42-0.55)
c_H     = 2;      % Pandemic health cost per unit infection (2-4)
...
% --- 1. Initialisierung & Szenario-Setup ---
x0 = [0; 0; 0; 0.01]; 
max_iter  = 100;       
tol       = 1e-7;      

%5Hier auch definieren wie sich die Ansteckung mit den Wellen verändert hat

% Schocks (Wellen) definieren
epsilon_vec = zeros(4, N);
epsilon_vec(4, 4) = 0.001; % Beispiel: +5% Infektionen in Q4 %%Neue Infektion in QUartal K
epsilon_vec(4, 6) = 0.001; % Beispiel: +10% Infektionen in Q6 

% Impfstoff-Parameter
delta_theta_vec = 0.01 * ones(1, N); %Tödlichkeit
rho_theta_vec   = 1.3 * ones(1, N); %R-Rate
vaccine_q = 12; 
delta_theta_vec(vaccine_q:end) = 0.00000005; % Todesschutz
rho_theta_vec(vaccine_q:end)   = 0.6;    % Herdenimmunität



% --- 2. Dynamik-Funktionen (NUR EINMAL DEFINIEREN!) ---
% WICHTIG: Hier ist das + epsilon_vec(4,k) fest eingebaut
f_nonlin = @(x, u, k) [
    (rho_y + psi*u(1) - eta*u(3)) * x(1) - alpha_S*u(1) + alpha_F_DI*u(2);
    delta_theta_vec(k) * x(4);
    (1+r)*x(3) - gamma_y*x(1) + kappa_F_DI*u(2) + kappa_F_CP*u(3) + c_H*x(4);
    rho_theta_vec(k) * (1 - phi_S*u(1)) * x(4) + epsilon_vec(4, k)
];

compute_Ak = @(x_bar, u_bar, k) [
    rho_y + psi*u_bar(1) - eta*u_bar(3),  0,  0,  0;
    0,                                      0,  0,  delta_theta_vec(k);
    -gamma_y,                               0,  (1+r),  c_H;
    0,                                      0,  0,  rho_theta_vec(k)*(1 - phi_S*u_bar(1))
];
...
compute_Bk = @(x_bar, u_bar, k) [
    ...
    -rho_theta_vec(k)*phi_S*x_bar(4), 0,              0
];
...
%% ========================================================================
%  ITERATIVE LQR (iLQR) mit PROJECTED FEEDFORWARD & IMPFUNG
% =========================================================================
...
% --- Main iLQR loop ---
for iter = 1:max_iter
    % --- Step 2: Compute time-varying Jacobians ---
    Ak = cell(N, 1);
    Bk = cell(N, 1);
    for k = 1:N
        % HIER SIND DAS ZWEITE UND DRITTE 'k' ERGÄNZT:
    ...
    for k = N:-1:1
        ...
        u_target = u_bar(:, k) + k_ff;
        %u_target(1) = max(0, min(1,   u_target(1))); 
        %u_target(2) = max(0, min(0.2, u_target(2))); 
        %u_target(3) = max(0, min(0.2, u_target(3))); 
        
        k_ff_clamped{k} = u_target - u_bar(:, k);

        p{k} = Qx + K{k}' * Quu * k_ff_clamped{k};
        P{k} = Qxx - K{k}' * Quu * K{k};
    end

    % --- Step 4: Forward simulation with Line Search ---
    alpha = 1.0; 
    cost_old = compute_welfare(x_bar, u_bar, Q_base, Q_N, R, beta, N);
    ...
    while alpha > 1e-4
        ...
        for k = 1:N
            u_new(:, k) = u_bar(:, k) + alpha * k_ff_clamped{k} - K{k} * (x_new(:, k) - x_bar(:, k));

            u_new(1, k) = max(0, min(1,   u_new(1, k)));    %%HARTE GRENZEN
            u_new(2, k) = max(0, min(0.2, u_new(2, k)));            
            u_new(3, k) = max(0, min(0.2, u_new(3, k)));            

            % HIER IST DAS VIERTE FEHLENDE 'k' ERGÄNZT:
            x_new(:, k+1) = f_nonlin(x_new(:, k), u_new(:, k), k); 
        end

        cost_new = compute_welfare(x_new, u_new, Q_base, Q_N, R, beta, N);
    ...
    if ~line_search_success
        fprintf('  Line Search gescheitert. Lokales Optimum oder Schranken erreicht.\n');
        break;
    end
    ...
    cost_history(iter) = cost_new;

    if iter <= 5 || mod(iter, 10) == 0 || delta_x < tol
        fprintf('  Iter %3d:  alpha = %.3f,  ||dx||/||x|| = %.2e,  J = %.6f\n', ...
                iter, alpha, delta_x, cost_new);
    end
    ...
    % --- Step 6: Update Reference Trajectory ---
    x_bar = x_new;
    u_bar = u_new;
end

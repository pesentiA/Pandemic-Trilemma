%% ========================================================================
%  PANDEMIC TRILEMMA - CALIBRATION V16 (main COVID period, Option B,
%  wave-specific delta_theta)
%
%  CHANGES vs V15:
%   (1) DEBT VALIDATION now tracks BOTH the level and the quarterly CHANGE.
%       The level RMSE/SD ratio (~0.06 in V15) is mechanical: b starts at
%       the imposed b0 and r~=0, so the simulated level is ~b0+cumsum(flows)
%       and the ratio is dominated by cross-country b0 dispersion, not by
%       fiscal dynamics. The CHANGE-based RMSE (on debt_dR) is the honest
%       metric for whether the fiscal pass-throughs reproduce debt DYNAMICS.
%       Both are reported; the checklist now keys on the change metric.
%   (2) DEBT ROLL now includes the exogenous HEALTH term kappa_H * F_H,
...
%       takeup_guar * F_CP_guar in MATLAB (line ~184), 0.25x.
%
...
alpha_DI_lag1 =  1.470;
alpha_S_DI    = -0.041;
beta_d        =  0.0;          % fear term: NOT separately identified at quarterly
                               % frequency (weak in estimation); behavioral channel
                               % absorbed in S and rho_y. Set to 0 (see paper text).
...
rho_theta   =  1.035;   %Aus der eigenen Schätzung
...
%% ========================================================================
%  STEP 5b: THETA DIAGNOSTIC (shock-free) -- OPTIMIZATION READINESS CHECK
% =========================================================================
%  The main roll reproduces theta_obs exactly because eps_theta is built as
%  (theta_obs - expected), i.e. it back-fills whatever rho_theta/phi_S miss.
%  That is fine for CALIBRATION (theta is descriptive) but tells us nothing
%  about whether the theta TRANSITION alone is good enough for OPTIMIZATION,
%  where the planner re-chooses S and theta must respond endogenously via phi_S.
%
%  This block rolls theta WITHOUT the reproducing shocks: only the Q1.2020
%  initial seed is kept (the genuine exogenous import). If shock-free theta
%  tracks theta_obs in SHAPE, rho_theta/phi_S are optimization-ready. If it
%  diverges badly, the planner would optimize on an unrealistic theta path
%  and rho_theta/phi_S must be recalibrated before the optimization step.
%
%  NOTE: purely diagnostic. Does NOT alter the calibration above.
% =========================================================================
fprintf('\n========================================\n');
fprintf('  STEP 5b: Theta diagnostic (shock-free)\n');
fprintf('========================================\n');

for i = 1:n_c
    th_free = zeros(1, N);
    % Seed at the FIRST quarter with positive observed theta (the genuine
    % pandemic onset), not Q4.2019 where theta_obs = 0 by definition.
    % A zero seed in a multiplicative transition stays zero forever.
    k0 = find(cdata(i).theta_obs > 0, 1, 'first');
    if isempty(k0), cdata(i).theta_free = th_free;
                    cdata(i).rmse_theta_free = 0;
                    cdata(i).theta_free_corr = NaN; continue; end
    th_free(k0) = cdata(i).theta_obs(k0);
    for k = (k0+1):N
        Sk_norm = cdata(i).S(k) / 100;
        th_free(k) = P.rho_theta * (1 - P.phi_S * Sk_norm) * th_free(k-1);
        if th_free(k) < 0, th_free(k) = 0; end
    end
    cdata(i).theta_free = th_free;
    cdata(i).rmse_theta_free = sqrt(mean( ...
        (th_free(1:K_theta) - cdata(i).theta_obs(1:K_theta)).^2));
    to = cdata(i).theta_obs(1:K_theta);
    tf = th_free(1:K_theta);
    if std(to) > 1e-12 && std(tf) > 1e-12
        cc = corrcoef(to, tf); cdata(i).theta_free_corr = cc(1,2);
    else
        cdata(i).theta_free_corr = NaN;
    end
end

fprintf('  Shock-free theta RMSE  -- Median: %.5f   Mean: %.5f\n', ...
    median([cdata.rmse_theta_free]), mean([cdata.rmse_theta_free]));
fprintf('  (compare to with-shock theta RMSE: ~0, which is mechanical)\n');
fprintf('  Shape corr(theta_free, theta_obs) -- Median: %.3f   Mean: %.3f\n', ...
    median([cdata.theta_free_corr],'omitnan'), mean([cdata.theta_free_corr],'omitnan'));
fprintf('  Countries with shape corr > 0.5: %d / %d\n', ...
    sum([cdata.theta_free_corr] > 0.5), n_c);

% OECD-median trajectory: shock-free theta vs observed
fprintf('\n  OECD median theta: obs vs shock-free transition\n');
fprintf('  %8s %12s %12s\n', 'Quarter','theta_obs','theta_free');
for k = 1:K_theta
    to_med = median(arrayfun(@(c) c.theta_obs(k),  cdata));
    tf_med = median(arrayfun(@(c) c.theta_free(k), cdata));
    fprintf('  %8s %12.5f %12.5f\n', qlbl{k}, to_med, tf_med);
end
fprintf(['  READ: if theta_free collapses to ~0 or explodes vs theta_obs,\n' ...
         '  the transition cannot sustain the observed waves on its own ->\n' ...
         '  rho_theta/phi_S need recalibration before optimization.\n']);
...
sgtitle('Calibration V16 - Trilemma (y, b, \theta, d) | Wave 1','FontWeight','bold');
...
figure('Name','Debt change V16','Color','w','Position',[80 80 700 420]);
hold on;

...
%
%  Fixed effects:
%    mu_y_i  — country FE from output equation (N=418 spec)
%    mu_b_i  — country FE from debt equation
%    eps_y_k — quarter FE from output equation (Q4.2019 = 0 reference)
%
...
rho_y      = 0.372;       % beta(y_lag)   ×1
psi        = 0.200;       % beta(S*y)     ×100
alpha_S    = 0.016;       % -beta(S)      ×1
alpha_F_CP = 0.249;       % beta(FCP)     ×1
eta_tilde  = -0.700;      % beta(S*FCP)   ×100
eta_p      = 2.600;       % -beta(y*FCP)  ×100
alpha_F_DI = 0.224;       % beta(FDI_l2)  ×1
beta_fear  = -0.022;      % beta(d)       ×1
...
% --- Country FE: Output (38 countries, alphabetical, units: pp → /100) ---
...
% --- Country FE: Debt (38 countries, DEMEANED, mean=0, units: pp → /100) ---
cfe_b_val = [-0.2386,  0.1584, -0.0289,  0.2218, -0.0484, -0.5421,  0.5823,  0.8408, ...
             -0.6259, -0.5926, -0.8179, -0.7911, -0.1470, -0.0987, -0.4048, -0.4614, ...
             -0.1736, -0.2344,  1.3804, -0.5975,  0.6691, -0.2119,  0.2338,  0.1369, ...
             -0.0720,  0.7297,  0.4148, -1.1938,  0.0138, -0.0042,  0.7814, -0.1988, ...
             -0.9959,  0.3719,  0.0334,  0.0581,  0.7490,  1.1041];
...
% --- Build FE lookup maps (fractions) ---
...
for i = 1:n_c
    ...
    mu_y_i  = cdata(i).mu_y;  mu_b_i = cdata(i).mu_b;
    fcp_obs = cdata(i).FCP;   fdi_obs = cdata(i).FDI;

    [us_il, J_il] = solve_ilqr_cfb(fcp_obs, fdi_obs, S_i, th_i, d_i, mu_y_i, mu_b_i, P);
    [us_ds, J_ds] = solve_direct_cfb(fcp_obs, fdi_obs, S_i, th_i, d_i, mu_y_i, mu_b_i, P, us_il);
    ...
    xs_ds = forward_roll(fcp_ds_f, fdi_ds_f, S_i, th_i, d_i, mu_y_i, mu_b_i, P);
    xs_il = forward_roll(fcp_il_f, fdi_il_f, S_i, th_i, d_i, mu_y_i, mu_b_i, P);
...
for s = 1:n_sel
    ...
    xs_obs = forward_roll(cdata(idx).FCP, cdata(idx).FDI, ...
        ...
        cdata(idx).mu_y, cdata(idx).mu_b, P);
...
function xs = forward_roll(fcp, fdi, S, theta, d, mu_y, mu_b, P)
...
function J = eval_J(fcp, fdi, S, theta, d, mu_y, mu_b, P)
    fcp_f=[fcp, zeros(1, P.N-length(fcp))];
    fdi_f=[fdi, zeros(1, P.N-length(fdi))];
    xs=forward_roll(fcp_f, fdi_f, S, theta, d, mu_y, mu_b, P);
    N_=P.N; J=0;
    for k=1:N_
        bd=P.beta_disc^(k-1);
...
function xn = dynamics_step(x, u, k, S, theta, d, mu_y, mu_b, P)
...
function [us_opt,J_opt] = solve_direct_cfb(fcp_obs,fdi_obs,S,theta,d,mu_y,mu_b,P,us_ilqr)
    Ka=P.K_act; tot=sum(fcp_obs(1:Ka))+sum(fdi_obs(1:Ka)); pen=1e4;
    obj=@(x) eval_J(max(0,x(1:Ka)),max(0,x(Ka+1:2*Ka)),S,theta,d,mu_y,mu_b,P) ...
    ...
    J_opt=eval_J(us_opt(1,:),us_opt(2,:),S,theta,d,mu_y,mu_b,P);
end

function [us_opt,J_opt] = solve_ilqr_cfb(fcp_obs,fdi_obs,S,theta,d,mu_y,mu_b,P)
    ...
    xs=zeros(nx_,N_+1);
    for k=1:N_
        xs(:,k+1)=dynamics_step(xs(:,k),us_f(:,k),k,S,theta,d,mu_y,mu_b,P);
    end
    ...
    for outer=1:max_outer
        for inner=1:max_inner
            ...
            J_old=eval_J(us_f(1,1:Ka),us_f(2,1:Ka),S,theta,d,mu_y,mu_b,P) ...
            ...
            while al>1e-10
                ...
                for k=1:N_
                    ...
                    xn(:,k+1)=dynamics_step(xn(:,k),un(:,k),k, ...
                        S,theta,d,mu_y,mu_b,P);
                end
                svn=sum(un(1,1:Ka))+sum(un(2,1:Ka))-tot;
                Jn=eval_J(un(1,1:Ka),un(2,1:Ka),S,theta,d,mu_y,mu_b,P) ...

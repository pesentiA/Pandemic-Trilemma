...
RUN.ita_relaxed_ramp     = true; % ITA-Sonderlauf: Frontier mit p99-/unbegrenzter
                                 % Ramp (Robustheits-Satz, app:country_special)
...
OB  = nan(nR, 18);  % Ly, cum_y, Dcum, dbT, avgS, totAb, totBel, totDI,
                    % below_share, DI_share, frontload, exceed_cap,
                    % clo_y, clo_d_ratio, clo_bT, exceed_dS,
                    % DI_share_highS, DI_share_lowS (S*-Zerlegung)
% frontier
...
for rr = 1:nR
    ...
    % DI-Zerlegung nach Containment-Zustand im WIRKUNGS-Quartal (k+1, da DI
    % gelagged wirkt): Anteil des Pakets, der als DI oberhalb/unterhalb der
    % Multiplikator-Schwelle S* = alpha_DI/|alpha_SDI| eingesetzt wurde.
    % Modell-Vorhersage: XR korreliert stark negativ mit DI_high, nicht mit
    % DI_low (eingebautes Placebo).
    Sstar = P.alpha_DI / abs(P.alpha_SDI);
    di_high = 0; di_low = 0;
    for k = idx_eval(1:end-1)                    % k=2..12; Wirkung bei k+1 im Fenster
        if U_obs(1,k+1) > Sstar
            di_high = di_high + U_obs(5,k);
        else
            di_low  = di_low  + U_obs(5,k);
        end
    end
    if totAll > 1e-6
        DI_high_pkg = di_high/totAll; DI_low_pkg = di_low/totAll;
    else
        DI_high_pkg = NaN; DI_low_pkg = NaN;
    end
    OB(rr,:) = [Mobs.Ly, Mobs.cum_y, Mobs.Dcum, Mobs.bT - Ci.b0, ...
                mean(U_obs(1,idx_eval)), totAb, totBel, totDI, ...
                below_share, DI_share, frontload, exceed_cap, ...
                clo_y, clo_d, clo_bT, exceed_dS, DI_high_pkg, DI_low_pkg];
...
T_ob = array2table(OB, 'VariableNames', ...
    {'Ly_obs','Cum_y_obs','Dcum_obs','Delta_b_obs','Avg_S_obs', ...
     ...
     'Obs_exceeds_dS_max','DI_share_highS','DI_share_lowS'});
...
% ---- DI-Schwellen-Test (Punkt 1): XR gegen DI-Zerlegung -------------------
xr_v0 = CF(:,6);
okx = isfinite(xr_v0) & isfinite(OB(:,10));
fprintf('\nDI-Schwellen-Test (S* = %.1f, Wirkungs-Quartal):\n', P.alpha_DI/abs(P.alpha_SDI));
fprintf('  corr(XR, DI_share gesamt)   = %+.2f\n', corr(OB(okx,10), xr_v0(okx)));
okh = isfinite(xr_v0) & isfinite(OB(:,17));
fprintf('  corr(XR, DI_share high-S)   = %+.2f   <- Modell-Vorhersage: stark negativ\n', ...
        corr(OB(okh,17), xr_v0(okh)));
okl = isfinite(xr_v0) & isfinite(OB(:,18));
fprintf('  corr(XR, DI_share low-S)    = %+.2f   <- Placebo: schwach/null\n', ...
        corr(OB(okl,18), xr_v0(okl)));
...
%% ------------------------------------------------------------------------
%  ITA-SONDERLAUF: gelockerte Ramping-Constraint                 [V23 NEU]
% -------------------------------------------------------------------------
% Italien ist das einzige Land, dessen beobachtete Politik am gemeinsamen
% Menue scheitert - pivotal ist die Ramp-Geschwindigkeit (p95). Dieser Block
% loest die drei ITA-Programme einmal mit Ramp am 99. Perzentil und einmal
% ohne Ramping-Constraint. NUR Robustheits-Satz fuer app:country_special;
% verletzt die Gemeinsames-Menue-Vergleichbarkeit und gehoert NICHT in die
% Ranking-Tabelle.
if RUN.ita_relaxed_ramp
    i = find(strcmp(countries, 'ITA'));
    rr_ita = find(idx_run == i);
    if ~isempty(i) && ~isempty(rr_ita)
        dS_p99 = pctile(dS_obs, 99);
        ita_maxramp = max(abs([S_o(i,1), diff(S_o(i,:))]));
        fprintf('\n=== ITA-Sonderlauf: max obs Ramp %.1f | p95 %.1f | p99 %.1f ===\n', ...
                ita_maxramp, P.dS_max, dS_p99);
        ...
        ramp_variants = {dS_p99, 'p99'; 1e6, 'unbounded'};
        for v = 1:size(ramp_variants,1)
            P_ita = P; P_ita.dS_max = ramp_variants{v,1};
            starts_f = planner_starts(U_obs, ub, P_ita);
            res = nan(1,3);
            try
                [~,~,MA_i] = solve_frontier_multistart('output', starts_f, x0, P_ita, Ci, Wi, lb, ub, Mobs, frontier);
                res(1) = 100*(Mobs.Ly - MA_i.Ly)/Mobs.Ly;
            catch, end
            try
                [~,~,MB_i] = solve_frontier_multistart('mortality', starts_f, x0, P_ita, Ci, Wi, lb, ub, Mobs, frontier);
                res(2) = 100*(Mobs.Dcum - MB_i.Dcum)/Mobs.Dcum;
            catch, end
            try
                [~,~,MC_i] = solve_frontier_multistart('debt', starts_f, x0, P_ita, Ci, Wi, lb, ub, Mobs, frontier);
                res(3) = Mobs.bT - MC_i.bT;
            catch, end
            fprintf('  Ramp %-9s: ImpA %.1f%% | ImpB %.1f%% | ImpC %.1f pp\n', ...
                    ramp_variants{v,2}, res);
            % Einordnung in die Baseline-Verteilung (hypothetische Raenge)
            if all(isfinite(res([1 3])))
                hypA = 1 + sum(FR(:,1) < res(1) & isfinite(FR(:,1)));
                hypC = 1 + sum(FR(:,7) < res(3) & isfinite(FR(:,7)));
                fprintf('    hypothetische Raenge im 37er-Feld: A #%d | C #%d | dist %.1f\n', ...
                        hypA, hypC, (hypA+hypC)/2);
            end
        end
    end
end
...
% --- Natural-units Figur 1: realized debt (raw) vs foregone output --------
fig = figure('Color','w','Position',[80 80 780 620]);
db_raw = OB(:,4) - OB(:,15);            % Delta_b model-implied minus Closure-Gap = raw
fy_abs = FR(:,2);                        % ImpA absolut (pp-Quartale liegengelassen)
scatter(db_raw, fy_abs, 28, [0.25 0.25 0.25], 'filled'); hold on;
text(db_raw+0.25, fy_abs, ISO, 'FontSize', 7, 'Color', [0.2 0.2 0.2]);
title('Realized debt increase and foregone output stabilization', 'FontWeight','normal');
xlabel('Realized debt increase (pp of GDP, raw data)');
ylabel('Foregone output stabilization at own frontier (pp-quarters)');
set(gca,'TickDir','out','Box','off','FontSize',9);
grid on; ax = gca; ax.GridAlpha = 0.12;
exportgraphics(fig, 'fig_countries_debt_vs_foregone_aerstyle.pdf','ContentType','vector');
exportgraphics(fig, 'fig_countries_debt_vs_foregone_aerstyle.png','Resolution',300);

% --- Natural-units Figur 2: realized output loss (raw) vs wasted debt -----
fig = figure('Color','w','Position',[80 80 780 620]);
cy_raw = OB(:,2) - OB(:,13);            % raw kumulativer Output-Gap
wd_pp  = FR(:,7);                        % ImpC in pp (verschwendete Schulden)
scatter(cy_raw, wd_pp, 28, [0.25 0.25 0.25], 'filled'); hold on;
text(cy_raw+1.0, wd_pp, ISO, 'FontSize', 7, 'Color', [0.2 0.2 0.2]);
title('Realized output loss and wasted debt', 'FontWeight','normal');
xlabel('Realized cumulative output gap (pp-quarters, raw data)');
ylabel('Wasted debt at own frontier: ImpC (pp of GDP)');
set(gca,'TickDir','out','Box','off','FontSize',9);
grid on; ax = gca; ax.GridAlpha = 0.12;
exportgraphics(fig, 'fig_countries_outputloss_vs_wasteddebt_aerstyle.pdf','ContentType','vector');
exportgraphics(fig, 'fig_countries_outputloss_vs_wasteddebt_aerstyle.png','Resolution',300);
...
% --- Schlagzeilen-Scatter 3: realized Debt (raw) vs foregone Output --------
% Natuerliche Einheiten: "was hat das Land bezahlt, was hat es bei diesem
% Preis und eigener Gesundheitslast liegengelassen". Zusaetzlich zum
% AC-Scatter; Debt-Constraint bleibt im Solver aktiv (nur Darstellung).
fig = figure('Color','w','Position',[80 80 780 620]);
db_raw = OB(:,4) - OB(:,15);          % Delta_b model-implied minus Closure = raw
fo_abs = FR(:,2);                      % ImpA absolut (pp-Quartale)
scatter(db_raw, fo_abs, 28, [0.25 0.25 0.25], 'filled'); hold on;
lbl2 = ["USA","NZL","AUS","JPN","DNK","CHL","SWE","NOR","CHE","MEX", ...
        "FRA","PRT","FIN","DEU","CAN","ISR","COL","IRL","TUR","GBR","ESP"];
for rr = 1:nR
    if ismember(ISO(rr), lbl2) && isfinite(db_raw(rr)) && isfinite(fo_abs(rr))
        text(db_raw(rr)+0.25, fo_abs(rr), ISO(rr), 'FontSize', 7, ...
             'Color', [0.2 0.2 0.2]);
    end
end
xlabel('Realized debt increase (pp of GDP, raw data)');
ylabel('Foregone output stabilization (pp-quarters, at own deaths and debt)');
title('The price paid and the stabilization left unexploited','FontWeight','normal');
set(gca,'TickDir','out','Box','off','FontSize',9);
grid on; ax = gca; ax.GridAlpha = 0.12;
exportgraphics(fig, 'fig_countries_debt_foregone_aerstyle.pdf','ContentType','vector');
exportgraphics(fig, 'fig_countries_debt_foregone_aerstyle.png','Resolution',300);

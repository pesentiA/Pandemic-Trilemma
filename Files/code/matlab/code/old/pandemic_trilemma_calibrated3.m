...
for i = 1:n_c
    ...
    if std(val(i).sim_y(1:K_eval)) > 1e-10 && std(val(i).obs_y(1:K_eval)) > 1e-10
        tmp = corrcoef(val(i).sim_y(1:K_eval), val(i).obs_y(1:K_eval));
        val(i).corr_y = tmp(1,2);
    else
        val(i).corr_y = NaN;
    end
end
...
fprintf('\n  Output Gap (pp):\n');
sorted_y = sort(rmse_y_all);
n_v = length(sorted_y);
p25_y = sorted_y(max(1, round(0.25*n_v)));
p75_y = sorted_y(max(1, round(0.75*n_v)));
fprintf('    RMSE  — Median: %.2f  Mean: %.2f  P25: %.2f  P75: %.2f\n', ...
    median(rmse_y_all), mean(rmse_y_all), p25_y, p75_y);
...
% Spearman rank correlation (no toolbox needed)
[~, ry1] = sort(sim_cum_y); [~, ry1] = sort(ry1); % ranks
[~, ry2] = sort(obs_cum_y); [~, ry2] = sort(ry2);
tmp = corrcoef(ry1, ry2); rho_rank_y = tmp(1,2);
n_rank = length(sim_cum_y);
t_stat_y = rho_rank_y * sqrt((n_rank-2)/(1-rho_rank_y^2+1e-12));
p_rank_y = 2 * (1 - tcdf_approx(abs(t_stat_y), n_rank-2));

[~, rb1] = sort(sim_cum_b); [~, rb1] = sort(rb1);
[~, rb2] = sort(obs_cum_b); [~, rb2] = sort(rb2);
tmp = corrcoef(rb1, rb2); rho_rank_b = tmp(1,2);
t_stat_b = rho_rank_b * sqrt((n_rank-2)/(1-rho_rank_b^2+1e-12));
p_rank_b = 2 * (1 - tcdf_approx(abs(t_stat_b), n_rank-2));
...
% =========================================================================
w_y = 100;   r_S = 10;   r_DI = 5;   r_CP = 2.0;
...
%  Extended MCPF range to generate visible spread in debt dimension
VSL_grid  = [10 20 40 60 80 100 140];
MCPF_grid = [0.1 0.5 1.0 3.0 5.0 10.0];
...
sel_v = [140 60 20 100 10]; sel_m = [0.1 1.0 10.0 5.0 0.1]; ns = length(sel_v);
si = zeros(ns,1);
for s = 1:ns
...
for i = 1:n_c
    ...
    %     Run from TWO initializations, pick lower J:
    %     (i) observed FDI/FCP, (ii) default ramp-down
    %     This prevents convergence failures for unusual country profiles.
    u_obs_init = u_obs;  % [S; FDI; FCP] — S will be overridden by fS
    [xcA1, ucA1, JcA1] = solve_ilqr(w_y, w_d_cf, w_b_cf, W_b_cf, ...
                                      r_S, r_DI, r_CP, x0_i, P, fS, fTh, fD, u_obs_init);
    [xcA2, ucA2, JcA2] = solve_ilqr(w_y, w_d_cf, w_b_cf, W_b_cf, ...
                                      r_S, r_DI, r_CP, x0_i, P, fS, fTh, fD, []);
    if JcA1 <= JcA2
        xcA = xcA1; ucA = ucA1; JcA = JcA1;
    else
        xcA = xcA2; ucA = ucA2; JcA = JcA2;
    end
    ...
    % Safeguard: if optimizer couldn't beat observed policy, flag it
    if JcA > J_obs
        fprintf('[WARN: comp_gap<0, using obs as floor] ');
        xcA = x_obs; ucA = u_obs; JcA = J_obs;
    end
...
%    u_init               — initial control trajectory [3xN] (empty = default)
%
%  Outputs:
...
function [x_opt, u_opt, J_opt] = solve_ilqr(w_y, w_d, w_b, W_b, ...
                                             r_S, r_DI, r_CP, x0, P, fS, fTh, fD, u_init)
    ...
    doTh = ~isempty(fTh);
    doD  = ~isempty(fD);
    doInit = nargin >= 13 && ~isempty(u_init);
    ...
    for k = 1:N
        if doInit
            % Use provided initial controls
            ub(:,k) = u_init(:,k);
            if doS, ub(1,k) = fS(k); end
        else
    ...
    max_outer = 30; max_inner = 100;
    tol_inner = 1e-5; tol_outer = 1e-5;
    penalty_growth = 2.5; reg_base = 1e-3;
...
% --- Approximate t-distribution CDF (no toolbox needed) ---
%  Uses incomplete beta function (base MATLAB) for exact computation.
function p = tcdf_approx(t, v)
    x = v ./ (v + t.^2);
    p = 1 - 0.5 * betainc(x, v/2, 0.5);
end

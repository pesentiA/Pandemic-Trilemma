%% ========================================================================
%  PANDEMIC TRILEMMA - CALIBRATION V19 (main COVID period, Option B,
...
%   (B) wave_idx_q aligned to solver V17:
%       [1 1 2 3 3 4 5 6 6 6 7 7 7]  ->  [1 1 2 3 4 4 5 6 6 7 7 7 7].
%       This changes delta_theta in Q4.20 (W2_wt 0.7% -> W2_alpha 0.4%) and
%       Q1.22 (W4 0.2% -> W4_omicron 0.02%); both fall inside the targeted
%       d-window (k=5 and k=10 <= K_theta=11) and therefore affect the
%       simulated death path in those two quarters.
%
...
%       Both are reported; the checklist keys on the change metric.
...
%       takeup_guar * F_CP_guar in MATLAB, 0.25x.
%
...
% =========================================================================
clear; clc; close all;
fprintf('=== PANDEMIC TRILEMMA: Calibration V19 ===\n');
fprintf('  %s\n\n', datestr(now));
...
% rho_theta is quarter-varying with a vaccination break (matches solver V17).
rho_theta_pre  =  1.5;     % pre-vaccination (Aus der eigenen Schätzung)
rho_theta_post =  0.5;     % post-vaccination (de Gier et al. 2021 + coverage)
q_vax          =  8;       % break quarter (Q3.2021)
phi_S          =  0.8;     % S in [0,1]  (matches solver V17)

% Wave-specific IFR (matches solver V17):
%   W1=0.5%  W1_summer=0.7%  W2_wt=0.5%  W2_alpha=0.4%  W3_delta=0.25%  W4_omicron=0.028%
ifr_by_wave         = [0.005 0.002 0.007 0.004 0.003 0.002 0.0002];  %%Quellen angeben im Text!

%% ========================================================================
%  PANDEMIC TRILEMMA - iLQR SOLVER, AVERAGE OECD ECONOMY (MATLAB)
%  ------------------------------------------------------------------------
...
% =========================================================================
clear; clc; close all;
fprintf('=== iLQR PLANNER: AVERAGE OECD ECONOMY ===\n  %s\n\n', datestr(now));

%% ------------------------------------------------------------------------
%  Calibrated V15 parameters
% -------------------------------------------------------------------------
P.rho_y=0.231;  P.alpha_S=-0.0952; P.alpha_above=0.544; P.alpha_below=0.261165;
P.alpha_DI=1.470; P.alpha_SDI=-0.0406; P.beta_d=0.0115;
P.c_lo=0.40; P.c_gu=0.25/0.35;
P.r=0.001; P.gamma_y=0.176;
P.k_ab=0.392; P.k_lo=0.891; P.k_gu=0.111; P.k_di=0.396; P.phi_t=-0.076;
P.rho_th=1.035; P.phi_S=0.314;
ifr=[0.009 0.007 0.006 0.004 0.003 0.0004];
P.delta_q=ifr([1 1 1 2 3 3 4 5 5 6 6 6 6])*1e6;     % wave-indexed deltas
P.N=13; P.yr=4:16;
P.n=8; P.m=5;                                       % state / control dims
...
for i = 1:n_c
    for k = 2:N
        expct = P.rho_th*(1-P.phi_S*S_o(i,k)/100)*th_o(i,k-1);
...
sgtitle('iLQR planner vs observed average OECD policy','FontWeight','bold');
...
% STEP 0: one-step Markovian transition; q = quarter index (1..N).
    ...
    st_used = st + P.c_lo*flo + P.c_gu*fgu;        % stock incl. current flow
    eth = 0; if q+1 <= P.N, eth = C.eps_th(q+2); end  % shock convention of V15
    xp = zeros(P.n,1);
    xp(3) = P.rho_th*(1-P.phi_S*S/100)*th + eth;                   % theta
...
% Discounted quadratic trilemma loss (state cost attaches to arrival x_q).
    J = 0;
    for q = 1:P.N
        x = X(:,q+1); u = U(:,q);
...
% Trajectory-dependent entries come from the bilinear terms S*theta, S*di1.
    S=u(1); th=x(3); di1=x(7);
    A = zeros(P.n); B = zeros(P.n,P.m);
    A(1,1)=P.rho_y; A(1,4)=-P.beta_d; A(1,6)=P.alpha_above;
    A(1,7)=P.alpha_DI + P.alpha_SDI*S; A(1,8)=P.alpha_below;
    ...
    A(3,3)=P.rho_th*(1-P.phi_S*S/100);
    B(3,1)=-P.rho_th*P.phi_S/100*th;
    A(4,3)=P.delta_q(q);
    A(6,5)=1; A(8,8)=1;

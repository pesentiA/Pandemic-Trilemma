...
excl_iso = {'TUR', 'IRL'};                           % leer = alle 38; zum Testen z.B. {'MEX','COL','CRI'}
if ~isempty(excl_iso)
...
cdata(i).FCP_loans_adj   = takeup_loans * cdata(i).FCP_loans;   % for DEBT (take-up)
cdata(i).FCP_guar_adj    = takeup_guar * cdata(i).FCP_guar;     % for DEBT (take-up)
cdata(i).FCP_below_flow  = cdata(i).FCP_loans_adj + cdata(i).FCP_guar_adj;
cdata(i).FCP_below_stock = cumsum(cdata(i).FCP_loans + cdata(i).FCP_guar);  % for OUTPUT: FACE VALUE (announced)
...
%     - output shock eps_y split 0.08/0.92 across Q1.20/Q2.20 (solver
%       convention), not a single Q2.20 shock;
...
% --- output shock: solver split 0.08 / 0.92 across Q1.20 / Q2.20 ----------
mean_eps_v14 = mean(arrayfun(@(c) c.eps_y_vec(4), cdata));
fprintf('  mean_eps_v14 (kept sample): %.3f\n', mean_eps_v14);
SC.eps_y = zeros(1, N+1);
SC.eps_y(3) = 0.08 * mean_eps_v14;            % Q1.20 onset
SC.eps_y(4) = 0.92 * mean_eps_v14;            % Q2.20 through

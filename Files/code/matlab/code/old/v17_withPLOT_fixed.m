...
rho_theta_pre  = 1.5;   rho_theta_post = 0.5;   q_vax = 8; %Aus der Literatur-> Checken wenn auf 2.5 einfach mehr Lockdown-> FUNKTIONIERT->%halbierung der rate durch impfung-> de Gier VET + Impfquote # Central estimate (Brauner et al. 2021; Flaxman et al. 2020) phi_S_range   <- c(0.60, 0.82)  # Range from Calibration Report
...
ifr=[0.005 0.002 0.007 0.004 0.003 0.002 0.0002];  %[0.005 0.005 0.004 0.003 0.002 0.0002]-> from data with lo-> aus der Literatur
P.delta_q=ifr([1 1 2 3 4 4 5 6 6 7 7 7 7])*1e6;
P.N=13; P.yr=4:16;
...
W.w_b = 1/var(b_lvl(:),1)*tau^2;                % Schulden: relativ zu Output via tau
W.lam_d = 3;          % lam_d = freier Relativgewicht-Parameter (Sweep)
W.w_d = 1/var(d_o(:),1);             % Varianz-normiert, analog zu w_y-> fixiert
fprintf('  tau=%.1f => w_y/w_b=%.2f | lam_d=%.2f, w_d/w_y=%.3f\n', ...
        tau, W.w_y/W.w_b, W.lam_d, W.w_d/W.w_y);y_dm = y_o   - mean(y_o,2);      % within-country demeaned
...
%No sweep hier, they are only for the model so that it works-> show that
%the results are not sensitiv to that->REGULATOREN %FLAG
W.p_S = 0.02 * W.w_y; % [0.005-0.02]
W.p_F = 0.1/var(allF,1); %[0.02, 0.05, 0.1]

W.terminal_mode = 'riccati'; %mit 'riccati' wird auch Theta als Endzustand gewichtet (braucht keinen lam_cons parameter), mit 'debt_M' nur die Verschuldung (braucht lam_cons), oder 'none'
W.lam_cons = 0.95; %[0.95,0.98,0.99]-> gibt die präferenzen der schulden am ende an, barwert über alle horizonte , wenn tiefer mehr von den teuren F
...
plot(qs, X(4,2:N+1), 'b-o','LineWidth',2); hold on;
plot(qs, Xo(4,2:N+1),'r-^','LineWidth',1.5);
...
safe_export(fig, 'fig_planner_policy_paths_baseline_aerstyle');
...
% Export
safe_export(fig, 'fig_planner_outcome_paths_baseline_aerstyle');
...
plot_names = component_names(1:6);
plot_obs = obs_share(1:6);
plot_opt = opt_share(1:6);
...
safe_export(fig, 'fig_welfare_decomposition_baseline_aerstyle');
...
% Export
safe_export(fig, 'fig_welfare_gain_decomposition_baseline_aerstyle');
...
% NEU (Option A: positive Innovationen nur vor Vax; danach voll):
eth = 0;
if (q+2) <= numel(C.eps_th)
    eth = C.eps_th(q+2);
end
if q < P.q_vax           % vor Impfung: kein eingebackenes S-Abklingen
    eth = max(eth, 0);
end
    xp = zeros(P.n,1);
...
%       + beta^N (x_T-xbar)' P_inf (x_T-xbar)
    ...
    for q = 1:P.N
        ...
        JS = JS + dq * W.p_S * uq(1)^2;
        JF = JF + dq * W.p_F * sum(uq(2:5).^2);
    end
    ...
    Comp.Fiscal     = JF;
    Comp.Terminal   = JT;
    Comp.Total      = Jy + Jd + Jb + JS + JF + JT;
end

function safe_export(fig, filename_base)
% Robust figure export helper.
% Saves both PDF and PNG. Uses exportgraphics when possible and falls back
% to print if the graphics exporter becomes unstable.

    if ~isgraphics(fig, 'figure')
        warning('Figure handle is invalid. Skipping export for %s.', filename_base);
        return;
    end
    ...
    % --- PDF export ------------------------------------------------------
    try
        exportgraphics(fig, [filename_base '.pdf'], ...
            'ContentType', 'vector', ...
            'BackgroundColor', 'white');
    catch ME
        warning('PDF exportgraphics failed for %s: %s', filename_base, ME.message);
        if isgraphics(fig, 'figure')
            try
                set(fig, 'PaperPositionMode', 'auto');
                print(fig, [filename_base '.pdf'], '-dpdf', '-painters');
            catch ME2
                warning('PDF print fallback also failed for %s: %s', filename_base, ME2.message);
            end
        end
    end
    ...
    try
        if isgraphics(fig, 'figure')
            exportgraphics(fig, [filename_base '.png'], ...
                'Resolution', 300, ...
                'BackgroundColor', 'white');
        end
    catch ME
        warning('PNG exportgraphics failed for %s: %s', filename_base, ME.message);
        if isgraphics(fig, 'figure')
            try
                set(fig, 'PaperPositionMode', 'auto');
                print(fig, [filename_base '.png'], '-dpng', '-r300');
            catch ME2
                warning('PNG print fallback also failed for %s: %s', filename_base, ME2.message);
            end
        end
    end
end

...
% --- Output Dynamics ---
rho_y   = 0.8;     % Baseline persistence (Cerra et al., 2023) (0.85-0.95)
psi     = 0.1;     % Hysteresis sensitivity to lockdown (0.02-0.1)

% --- Epidemiology ---
rho_theta   = 1.3;  % Transmission rate (>1: explosive) (2.5-3)-> spätere Varaianten höher
delta_theta = 0.03;  % Infection fatality rate (0.008-0.010)

% --- Containment (S) ---
alpha_S = 0.1;      % Contractionary effect on output (0.08-0.15)
phi_S   = 0.45;      % Suppression effectiveness on infections (0.6-0.82)

% --- Demand Injection (F_DI) ---
alpha_F_DI = 0.3;   % Fiscal multiplier (0.3-1)->SELBER
kappa_F_DI = 0.3;   % Budgetary cost per unit (0.7-0.9)->SELBER

% --- Capacity Preservation (F_CP) ---
eta        = 0.4;   % Effectiveness on persistence reduction (0.03-0.08)->SELBER
kappa_F_CP = 0.01;   % Budgetary cost per unit (0.5-0.7)->SELBER

% --- Debt Dynamics ---
r       = 0.001;     % Quarterly real interest rate
gamma_y = 0.45;      % Budget semi-elasticity (auto-stabilizers) (0.42-0.55)
c_H     = 0;      % Pandemic health cost per unit infection (2-4)
...
% Weights
w_y = 50;    w_d = 1000000000;    w_b = 10;    W_b = 20;
r_S = 100; r_DI = 1000;   r_CP = 1000;
...
max_outer_iter = 15;      % Outer AL loop iterations
...
% Order: [-S; S-1; -F_DI; F_DI-0.2; -F_CP; F_CP-0.2]
C_u = [-1  0  0; 
        1  0  0; 
        0 -1  0; 
        0  1  0; 
...
function J = compute_al_cost(x_traj, u_traj, Q_base, Q_N, R, beta, N, lambda, mu, C_u, c_bounds)
    ...
    for k = 1:N
        % Base cost
        step_cost = beta^(k-1) * (x_traj(:,k)' * Q_base * x_traj(:,k) + u_traj(:,k)' * R * u_traj(:,k));

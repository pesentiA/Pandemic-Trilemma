...
% eps_th so bauen, dass es zur f_step-Rekursion passt:
eps_th = zeros(n_c, N+1);
for i = 1:n_c
    for q = 1:N                      % q = f_step Aufrufindex
        th_prev = 0; if q>1, th_prev = th_o(i,q-1); end   % = X(3,q)
        S_q = S_o(i,q);
        expct = P.rho_th_q(q)*(1-P.phi_S*S_q/100)*th_prev*(1-th_prev/P.th_max);
        eps_th(i,q+1) = th_o(i,q) - expct;     % Innovation für X(3,q+1)=th_raw(q)
    end
end
...
function xp = f_step(x, u, q, P, C)
    ...
    eth = 0; if (q+1) <= numel(C.eps_th), eth = C.eps_th(q+1); end
    xp = zeros(P.n,1);

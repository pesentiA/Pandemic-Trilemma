...
obs_b_all = reshape([cdata.obs_b_cum], K, n_c)' * 100;
...
for i = 1:n_c
    ...
    obj_A = @(s) eval_J_cf(s.*total_k, (1-s).*total_k, S, th, d, eps_y_vec, ...
        rho_y, psi, alpha_S, alpha_F_CP, eta_tilde, eta_p, alpha_F_DI, ...
        beta_fear, r_int, gamma_y, kappa_F_CP, kappa_F_DI, c_H, ...
        w_y, w_b, W_b, r_cp, r_di, beta_disc, K);
    
    % Multiple inits, pick best
    best_JA = Inf; best_sA = [];
    for s0_trial = [0.2, 0.5, 0.8, 0.95]
        s0 = s0_trial * ones(1,K);
        % Ensure init within bounds
        s0 = max(0, min(1, s0));
        opts = optimoptions('fmincon','Display','none','MaxIterations',3000,...
            'OptimalityTolerance',1e-10,'StepTolerance',1e-12);
        [s_opt, J_A_trial] = fmincon(obj_A, s0, [], [], [], [], ...
            zeros(1,K), ones(1,K), [], opts);
        if J_A_trial < best_JA
            best_JA = J_A_trial; best_sA = s_opt;
        end
    end
    J_A = min(best_JA, J_obs);  % safeguard
    fcpA = best_sA .* total_k; fdiA = (1-best_sA) .* total_k;
    ...
    obj_B = @(x) eval_J_cf(x(1:K), x(K+1:2*K), S, th, d, eps_y_vec, ...
        rho_y, psi, alpha_S, alpha_F_CP, eta_tilde, eta_p, alpha_F_DI, ...
        beta_fear, r_int, gamma_y, kappa_F_CP, kappa_F_DI, c_H, ...
        w_y, w_b, W_b, r_cp, r_di, beta_disc, K);
    con_B = @(x) deal([], sum(x(1:K)) + sum(x(K+1:2*K)) - total_cum);
    ...
    for trial = 1:3
        switch trial
            ...
            case 3, x0 = [fcpA, fdiA];  % start from CF-A solution
        end
        % Normalize to satisfy constraint
        sc = total_cum / max(sum(x0), 1e-10);
        x0 = x0 * sc;
        x0(1:K) = max(0, min(0.20, x0(1:K)));
        x0(K+1:2*K) = max(0, min(0.10, x0(K+1:2*K)));
        
        opts = optimoptions('fmincon','Display','none','MaxIterations',3000,...
            'OptimalityTolerance',1e-10,'StepTolerance',1e-12);
        [x_opt, J_B_trial] = fmincon(obj_B, x0, [], [], [], [], ...
            zeros(1,2*K), [0.20*ones(1,K), 0.10*ones(1,K)], con_B, opts);
        if J_B_trial < best_JB
            best_JB = J_B_trial; best_xB = x_opt;
        end
    end
    J_B = min(best_JB, J_A);  % safeguard monotonicity

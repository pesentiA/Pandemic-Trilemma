...
rho_y      = 0.26;     % Baseline persistence (p=0.051, now meaningful)
psi        = 0.003;
alpha_S    = 0.023;      alpha_F_CP = 0.262;
eta_tilde  = -0.023;     eta_p      = 0.023;    % Now significant (p=0.045)
alpha_F_DI = 0.195;      beta_fear  = -0.02;    % Coeff on P-score (pp→pp)
...
% --- Debt dynamics ---
r_int=0.001; gamma_y=0.21; kappa_F_CP=0.423; kappa_F_DI=0.37; c_H=0.02;
...
for i = 1:n_c
    ...
    cdata(i).d=zeros(1,N);  % observed P-score (excess mortality, %)
    for k = 1:N
        ...
        if ismember('excess_mortality', T.Properties.VariableNames) && ~ismissing(row.excess_mortality)
            cdata(i).d(k)=row.excess_mortality/100;  % P-score % → fraction
        end
    end
end
...
oecd.d=mean(reshape([cdata.d],N,n_c),2)';  % observed P-score (fraction)
...
for iv=1:nv, for im=1:nm
    ...
    par.cum_d(idx)=sum(xp(2,2:end))*baseline_qmort*100;  % excess deaths % of pop
...
titles_sub={'Containment (S)','Capacity Preservation (% GDP)','Demand Injection (% GDP)',...
            'Output Gap (pp GDP)','Excess Mortality (P-score, %)','Public Debt (pp GDP)'};

% Pre-build bar matrices for CP and DI [N x ns]
...
for p=1:6, subplot(2,3,p); hold on;
    switch p
        case 1  % Containment — lines
            for s=1:ns
                plot(1:N, par.u{si(s)}(1,:), '-', 'Color', cols(s,:), 'LineWidth', 1.8);
            end
            plot(1:N, oecd.S, 'k:', 'LineWidth', 2);
            ylabel('S [0,1]'); ylim([0 1]);
            set(gca,'XTick',1:N,'XTickLabel',qlbl,'XTickLabelRotation',45,'FontSize',6);

        case 2  % CP — grouped bars + OECD line
            bh = bar(1:N, bar_CP, 'grouped');
            for s=1:ns, bh(s).FaceColor = cols(s,:); bh(s).FaceAlpha = 0.7; end
            plot(1:N, oecd.FCP*100, 'k:o', 'LineWidth', 2, 'MarkerSize', 4);
            ylabel('% GDP');
            set(gca,'XTick',1:N,'XTickLabel',qlbl,'XTickLabelRotation',45,'FontSize',6);

        case 3  % DI — grouped bars + OECD line
            bh = bar(1:N, bar_DI, 'grouped');
            for s=1:ns, bh(s).FaceColor = cols(s,:); bh(s).FaceAlpha = 0.7; end
            plot(1:N, oecd.FDI*100, 'k:o', 'LineWidth', 2, 'MarkerSize', 4);
            ylabel('% GDP');
            set(gca,'XTick',1:N,'XTickLabel',qlbl,'XTickLabelRotation',45,'FontSize',6);

        case 4  % Output gap — lines
            for s=1:ns
                plot(0:N, par.x{si(s)}(1,:)*100, '-', 'Color', cols(s,:), 'LineWidth', 1.8);
            end
            plot(1:N, oecd.y*100, 'k:', 'LineWidth', 2);
            yline(0,'--k'); ylabel('pp GDP');

        case 5  % Excess mortality flow (P-score per quarter)
            for s=1:ns
                plot(1:N, par.x{si(s)}(2,2:end)*100, '-', 'Color', cols(s,:), 'LineWidth', 1.8);
            end
            plot(1:N, oecd.d*100, 'k:', 'LineWidth', 2);
            ylabel('P-score (%)'); yline(0,'--k');

        case 6  % Public debt — lines
            for s=1:ns
                plot(0:N, par.x{si(s)}(3,:)*100, '-', 'Color', cols(s,:), 'LineWidth', 1.8);
            end
            oecd_cum_b = [0, cumsum(oecd.b)] * 100;
            plot(0:N, oecd_cum_b, 'k:', 'LineWidth', 2);
            ylabel('pp GDP');
    end
...
for i=1:n_c
    od=sum(res(i).x_obs(2,2:end))*baseline_qmort*100;  % excess deaths % pop

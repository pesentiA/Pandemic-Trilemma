...
%Mit wihtin de-meaned variance kalibrierung
W.beta = 0.99;  %fixiert
y_dm = y_o   - mean(y_o,2);
d_dm = d_o   - mean(d_o,2);
W.w_y = 1/var(y_dm(:),1);              % Varianz-normiert-> fixiert
tau   = 0.02; %1.086= y 1:1
% within-country demeaned debt level
b_dm  = b_lvl - mean(b_lvl, 2);          % jedes Land um seinen Zeitmittelwert zentriert
W.w_b = 1/var(b_dm(:), 1) * tau;         % within-Varianz statt Gesamtvarianz
W.lam_d = 5;  %19.9=y 1:1        % lam_d = freier Relativgewicht-Parameter (Sweep)
W.w_d = 1/var(d_dm(:),1);             % Varianz-normiert, analog zu w_y-> fixiert
fprintf('  tau=%.3f => w_y/w_b=%.2f | lam_d=%.2f, w_d/w_y=%.3f\n', ...
...
% Observed average policy in effective units. Fl and Fg were converted from
% headline values to effective/take-up-adjusted values during data loading.
...
% persistent loans/guarantees stock. This is an objective component, NOT
...
fprintf('  stock penalty: p_stock=%.4f | stock_scale=%.3f\n', W.p_stock, W.stock_scale);
...
T_summary = table( ...
    ["Cumulative output gap"; ...
     ...
     "Total effective below-the-line liquidity"; ...
...
fprintf('\nControl-friction and stock-risk share of objective:\n');
fprintf('  Planner:  S-friction = %.4f (%.3f%%) | fiscal friction = %.4f (%.3f%%) | stock risk = %.4f (%.3f%%)\n', ...
        Comp_opt.Stringency, 100*Comp_opt.Stringency/Comp_opt.Total, ...
        Comp_opt.Fiscal,     100*Comp_opt.Fiscal/Comp_opt.Total, ...
        Comp_opt.Stock,      100*Comp_opt.Stock/Comp_opt.Total);
fprintf('  Observed: S-friction = %.4f (%.3f%%) | fiscal friction = %.4f (%.3f%%) | stock risk = %.4f (%.3f%%)\n', ...
        Comp_obs.Stringency, 100*Comp_obs.Stringency/Comp_obs.Total, ...
        Comp_obs.Fiscal,     100*Comp_obs.Fiscal/Comp_obs.Total, ...
        Comp_obs.Stock,      100*Comp_obs.Stock/Comp_obs.Total);
...
plot_names = component_names(1:end-1);
plot_obs = obs_share(1:end-1);
plot_opt = opt_share(1:end-1);
...
function xp = f_step(x, u, q, P, C)
    ...
    % below-the-line effective liquidity stock
    st_used = (1-P.decay_K)*st + flo + fgu;
...
% Controls are in effective units for loans/guarantees. Therefore c_lo and
% c_gu must NOT appear in the dynamics here.
    ...
    B(1,1)=P.alpha_S; B(1,3)=P.alpha_below; B(1,4)=P.alpha_below;

    A(2,1)=-P.gamma_y; A(2,2)=1+P.r; A(2,7)=P.k_di;
    B(2,2)=P.k_ab; B(2,3)=P.k_lo; B(2,4)=P.k_gu;

    A(3,3)=P.rho_th_q(end);
    A(4,3)=P.delta_q(end);
    A(6,5)=1; A(8,8)=1-P.decay_K;
    B(5,2)=1; B(7,5)=1; B(8,3)=1; B(8,4)=1;
...
%        + lam_d w_d d_q^2
%        + w_b (b_q-b0)^2
%        + p_u(j) * (u_j/u_scale_j)^2
%        + p_stock * (stock_q/stock_scale)^2
%       ]
%       + beta^N (x_T-xbar)' P_inf (x_T-xbar)
    ...
    for q = 1:P.N
        ...
        % Running stock-risk cost on below-line liquidity stock x(8)
        JStock = JStock + dq * W.p_stock * (xq(8) / W.stock_scale)^2;

        % Control/friction components, normalized by instrument scale
        JS = JS + dq * W.p_u(1) * (uq(1) / W.u_scale(1))^2;

        uF = uq(2:5);
        pF = W.p_u(2:5);
        sF = W.u_scale(2:5);
        JF = JF + dq * sum(pF(:) .* (uF(:) ./ sF(:)).^2);
    end

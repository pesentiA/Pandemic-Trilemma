...
%  V17 = V16 + TERMINAL VALUE Phi(x_N). No above x below interaction.
%  V17e: Q4.2019 fiscal lockout — the planner cannot spend before the shock
%        exists (theta=0 in Q4.19; first shock in Q2.20). Fiscal response may
%        begin in Q1.2020 (first real-world reactions, March 2020). Budget is
%        NOT fixed, so the planner can move the OUTPUT path (volume + comp).
%        Box bounds are now time-varying (m x N): column 1 = 0.
%
%  Control u = [F_above; F_loans; F_guar; F_DI]  (S exogenous, given).
%  Requires: country_data_for_matlab.csv, weekly_mortality_matlab.csv
% =========================================================================
clear; clc; close all;
fprintf('=== iLQR PLANNER V17e (Q4.19 lockout, output movable): AVERAGE OECD ===\n  %s\n\n', datestr(now));
...
sgtitle('iLQR planner V17e (Q4.19 lockout) vs observed average OECD','FontWeight','bold');

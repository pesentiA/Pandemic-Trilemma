...
%
%  HEALTH (F_H): exogenous, debt-only, one-quarter lag. NOT a control and NOT
%  in output/theta. Matches calibration V16 (kappa_H * F^H_{k-1}). The average
%  OECD health path is read from the CSV (F_H column); if absent it is zero.
%
...
P.alpha_SDI=-0.041;
P.beta_d=0.0;
P.c_lo=0.40;               % loans take-up (matches calib takeup_loans=0.60)
P.c_gu=0.25;               % guarantees take-up (matches calib takeup_guar=0.25)
P.r=0.001;
P.gamma_y=0.117;
P.k_ab=0.664;
P.k_lo=0.836;
P.k_gu=0.536;              % guarantee debt pass-through (matches calib kappa_guar)
P.k_di=0.526;              % DI debt pass-through (matches calib kappa_DI)
P.k_H =0.908;              % health debt pass-through (matches calib kappa_H)
P.phi_t= 0;

rho_theta_pre  = 1.035;   rho_theta_post = 0.65;   q_vax = 8; %halbierung der rate durch impfung-> de Gier VET + Impfquote # Central estimate (Brauner et al. 2021; Flaxman et al. 2020) phi_S_range   <- c(0.60, 0.82)  # Range from Calibration Report
P.rho_th_q = [repmat(rho_theta_pre,1,q_vax-1), ...
              repmat(rho_theta_post,1,13-q_vax+1)];
P.decay_K  = 0.1;
P.phi_S=0.6;
ifr=[0.005 0.009 0.005 0.004 0.0025 0.00028];  %[0.008 0.005 0.004 0.003 0.002 0.0002]-> from data with lo
...
[S_o,Fa,Fl,Fg,Fd,Fh,y_o,bd,th_o,d_o] = deal(zeros(n_c,N));
has_FH = ismember('F_H', T.Properties.VariableNames);
for i = 1:n_c
    iso = countries{i};
    for k = 1:N
        ...
        Fl(i,k)=row.F_CP_loans;  Fg(i,k)=row.F_CP_guar; Fd(i,k)=row.F_DI; % unadjusted guar (take-up applied via c_gu)
        y_o(i,k)=row.y_t_pct;
        if has_FH && ~ismissing(row.F_H), Fh(i,k)=row.F_H; end
...
C.b0 = mean(b0_val); C.mu_y = 0; C.mu_b = 0; %mean(cfe_b_val)
C.eps_y = zeros(1,N+1); C.eps_y(3) = mean(eps_v14);
C.FH = mean(Fh,1);                         % exogenous avg OECD health path (1xN)

dev  = b_lvl - b0_val';
allF = [Fa(Fa>0); Fl(Fl>0); Fg(Fg>0); Fd(Fd>0)];

% ---- planner weights: welfare-calibrated (option 2) ----------------------
W.beta = 0.99;  W.lam_d = 1; %FLAG: ganze Section, sauber machen->hängt vieles davon ab-> Verstehen und ausprobieren
W.w_y = 1/var(y_o(:),1); %Normiert, ist skalenfrei und unproblematisch
tau   = 5; %ANNAHME
W.w_b = W.w_y / tau^2;
...
nu      = val_d / val_y;
W.w_d   = W.w_y * nu; %Alternativ: Quadratisch machen-Y state dependent
...
function xp = f_step(x, u, q, P, C)
    ...
    % Exogenous health, one-quarter lag (matches calib kappa_H * F^H_{k-1}).
    fh_l1 = 0; if q >= 2 && q-1 <= numel(C.FH), fh_l1 = C.FH(q-1); end
    xp = zeros(P.n,1);
    xp(3) = P.rho_th_q(q)*(1-P.phi_S*S/100)*th + eth;
    xp(4) = P.delta_q(q)*th;
...
% Health enters debt only as an exogenous lagged term (no state/control
% dependence), so it contributes nothing to A or B.
...
% Health is exogenous (zero in the stationary deviation problem), so it does
% not enter A, B, Q, or R.
    A = zeros(P.n); B = zeros(P.n, P.m);
    A(1,1)=P.rho_y; A(1,4)=-P.beta_d; A(1,6)=P.alpha_above;

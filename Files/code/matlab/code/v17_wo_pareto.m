%% ========================================================================
%  PANDEMIC TRILEMMA - iLQR SOLVER V17g, AVERAGE OECD ECONOMY (MATLAB)
%  V17g = V17f + 2D WEIGHT-FRONTIER SWEEP.
%        In the exogenous-epidemic (Option A) model, deaths d and theta are
%        fixed data, so the planner solves a TWO-objective problem: output y
%        vs. debt b. Only the RATIO w_y:w_b matters (scaling both rescales J).
%        This version sweeps a debt-aversion multiplier lambda = w_b/w_b0 and
%        traces the achievable (cum. output gap, terminal debt) frontier,
%        overlaying the observed-average-policy point. The dominance claim is
%        weight-free if the observed point sits outside the frontier for all
%        lambda.
%
%        w_d is retained in the code but is INERT: it multiplies the fixed
%        deaths path, so it shifts J's level but never the argmin. It bites
%        again only once an endogenous mortality channel is restored.
%
%  V17f = V17e + EXOGENOUS EPIDEMIC BLOCK (Option A): theta,d = observed means.
...
% =========================================================================
clear; clc; close all;
fprintf('=== iLQR PLANNER V17g (exogenous epi; 2D weight frontier): AVERAGE OECD ===\n  %s\n\n', datestr(now));
...
W.beta=0.99; W.lam_d=1.0;  % w_d below is INERT in the exogenous-epi model
...
% --- baseline weights stored for the frontier sweep ---
w_y0 = W.w_y;   w_b0 = W.w_b;
fprintf('  baseline w_y/w_b ratio = %.3f\n', w_y0/w_b0);

% V17e: TIME-VARYING box bounds (m x N). Q4.19 (column 1) locked to zero;
% from q_start onward, the observed p95 cap applies.
ub_cap = [pctile(Fa(Fa>0),95); pctile(Fl(Fl>0),95); ...
...
%% ------------------------------------------------------------------------
%  BASELINE RUN (single weight = 1/var standardization)
% -------------------------------------------------------------------------
x0 = zeros(P.n,1); x0(2) = C.b0;
W.xbar = x0;
W = set_terminal(W, P);   % build W.P_inf for the current weights
...
sgtitle('iLQR planner V17g baseline (exogenous epi) vs observed average OECD','FontWeight','bold');

%% ------------------------------------------------------------------------
%  2D WEIGHT-FRONTIER SWEEP
%  Sweep lambda = w_b / w_b0 over a log grid; w_y fixed at w_y0.
%  lambda small  -> output-only planner (spend toward cap)
%  lambda large  -> debt-only planner   (F -> 0, should match zero-policy J*)
%  For each lambda: re-solve, record optimal composition, cum. output gap,
%  terminal debt. These (output, debt) pairs trace the 2D Pareto frontier.
% -------------------------------------------------------------------------
lambda = logspace(-1.5, 1.5, 25);   % w_b multiplier, ~0.032 .. ~31.6
nL = numel(lambda);
SW.lambda   = lambda;
SW.Jstar    = nan(1,nL);
SW.cumY     = nan(1,nL);     % cumulative output gap, sum(X(1,2:end))
SW.termDebt = nan(1,nL);     % terminal debt level X(2,end)
SW.dDebt    = nan(1,nL);     % terminal debt deviation from b0
SW.sumU     = nan(P.m,nL);   % optimal total spend per instrument

fprintf('\n--- 2D weight-frontier sweep (%d points) ---\n', nL);
for j = 1:nL
    Wj = W;
    Wj.w_y = w_y0;
    Wj.w_b = w_b0 * lambda(j);
    Wj = set_terminal(Wj, P);            % terminal value depends on weights

    % multistart at this lambda: observed policy and zero controls
    [Xa,Ua,Ja] = ilqr(U_obs,            x0, P, C, Wj, lb, ub);
    [Xb,Ub,Jb] = ilqr(zeros(P.m,P.N),   x0, P, C, Wj, lb, ub);
    if Ja <= Jb, Xs=Xa; Us=Ua; Js=Ja; else, Xs=Xb; Us=Ub; Js=Jb; end

    SW.Jstar(j)    = Js;
    SW.cumY(j)     = sum(Xs(1,2:end));
    SW.termDebt(j) = Xs(2,end);
    SW.dDebt(j)    = Xs(2,end) - C.b0;
    SW.sumU(:,j)   = sum(Us,2);

    fprintf('  lambda=%7.3f | J*=%8.4f | cumY=%+7.2f | dDebt=%+6.2f | F[ab lo gu DI]=[%5.2f %5.2f %5.2f %5.2f]\n', ...
        lambda(j), Js, SW.cumY(j), SW.dDebt(j), SW.sumU(1,j),SW.sumU(2,j),SW.sumU(3,j),SW.sumU(4,j));
end

% Observed-average-policy outcome (the point the frontier must dominate)
obs_cumY  = sum(Xo(1,2:end));
obs_dDebt = Xo(2,end) - C.b0;
fprintf('\n  Observed avg policy outcome: cumY = %+.2f ppQ, dDebt = %+.2f %%GDP\n', ...
        obs_cumY, obs_dDebt);

% Endpoint sanity check: large-lambda optimum should approach the F=0 (zero
% policy) trajectory cost. Report the gap.
[~,~,J_zero] = ilqr(zeros(P.m,P.N), x0, P, C, W, lb, ub);  %#ok<ASGLU>
fprintf('  [check] baseline-weight zero-policy optimum J* = %.4f\n', J_zero);

%% ------------------------------------------------------------------------
%  FRONTIER FIGURE
%  Axis convention: x = cumulative output gap (higher = better, less negative),
%                   y = terminal debt deviation (lower = better).
%  The frontier is the locus of optimal (output, debt) outcomes as lambda
%  varies. The observed point lies to the WORSE side (down-left) if dominated.
% -------------------------------------------------------------------------
figure('Color','w','Position',[80 80 1150 480]);

% Panel 1: the 2D Pareto frontier
subplot(1,2,1);
plot(SW.cumY, SW.dDebt, 'b-o', 'LineWidth', 2, 'MarkerFaceColor','b'); hold on;
plot(obs_cumY, obs_dDebt, 'rp', 'MarkerSize', 16, 'MarkerFaceColor','r');
% annotate lambda endpoints
text(SW.cumY(1),   SW.dDebt(1),   '  \lambda_{min} (output-only)', 'FontSize',8);
text(SW.cumY(end), SW.dDebt(end), '  \lambda_{max} (debt-only)',   'FontSize',8);
text(obs_cumY, obs_dDebt, '  observed avg', 'FontSize',9, 'Color','r');
xlabel('Cumulative output gap (ppQ)  \rightarrow better');
ylabel('Terminal debt deviation (%GDP)  \downarrow better');
title('2D Pareto frontier: output vs. debt');
grid on;

% Panel 2: optimal composition vs. lambda
subplot(1,2,2);
semilogx(lambda, SW.sumU(1,:), '-o', 'LineWidth',1.5); hold on;
semilogx(lambda, SW.sumU(2,:), '-s', 'LineWidth',1.5);
semilogx(lambda, SW.sumU(3,:), '-^', 'LineWidth',1.5);
semilogx(lambda, SW.sumU(4,:), '-d', 'LineWidth',1.5);
xlabel('\lambda = w_b / w_{b0}  (debt aversion)\rightarrow');
ylabel('Optimal cumulative spend  \Sigma_t F');
title('Optimal composition vs. debt aversion');
legend({'F_{above}','F_{loans}','F_{guar}','F_{DI}'}, 'Location','northeast');
grid on;

sgtitle('V17g weight-frontier sweep (2D, exogenous epidemic block)','FontWeight','bold');

%% ========================================================================
%  LOCAL FUNCTIONS
% =========================================================================

function W = set_terminal(W, P)
% Build W.P_inf for the CURRENT weights in W. Called once per weight setting.

...
P.alpha_above=0.544; 
P.alpha_below=0.261;
P.alpha_AB=0.130;   % V17c: above x below COMPLEMENTARITY (estimated:
                   % F_CP_above_flow_lag2 : F_CP_belowstock = 0.130, p=0.031).
                   % Above-the-line (worker support) and below-the-line (firm
                   % liquidity) reinforce each other; main effects insignificant
                   % alone -> they work jointly, not as substitutes. Prevents the
                   % planner from zeroing out above-the-line.
P.alpha_DI=1.470; 
P.alpha_SDI=-0.041;
P.beta_d=0.0;
P.c_lo=0.40; 
...
P.decay_K  = 0;        % below-the-line stock depreciation per quarter
...
% V17d: observed quarterly TOTAL fiscal budget (OECD mean, sum of 4 instruments).
% Used by the soft budget constraint: the planner re-allocates COMPOSITION at
% (approximately) the observed total spending per quarter, isolating the pure
% composition question from timing/level (which were not controllable ex ante).
C.B_obs = mean(Fa,1) + mean(Fl,1) + mean(Fg,1) + mean(Fd,1);   % 1 x N
...
% V17d: soft budget-constraint weight. Penalizes deviation of the quarterly
% control sum from the observed total C.B_obs(q). Set ~100x the output weight
% so the constraint binds tightly (composition free, total ~fixed). Tune up if
% residual budget deviation is too large; tune down if it harms conditioning.
W.lam_B = 100 * W.w_y;
...
fprintf('  %6s %6s %5s %5s %5s %5s | %6s %6s | %7s %7s %8s %6s\n', ...
        'Q','S(exo)','F_ab','F_lo','F_gu','F_DI','sumF','B_obs','y','b','theta','d');
for k = 1:P.N
    sumF = U(1,k)+U(2,k)+U(3,k)+U(4,k);
    fprintf('  %6s %6.1f %5.2f %5.2f %5.2f %5.2f | %6.2f %6.2f | %+7.2f %7.2f %8.5f %6.1f\n', ...
        qlbl{k}, C.S_exo(k), U(1,k),U(2,k),U(3,k),U(4,k), sumF, C.B_obs(k), ...
        X(1,k+1),X(2,k+1),X(3,k+1),X(4,k+1));
end
% V17d: budget-constraint diagnostic — how tightly does sumF track B_obs?
bud_dev = arrayfun(@(k) sum(U(:,k)) - C.B_obs(k), 1:P.N);
fprintf('\n  Budget constraint: max|sumF - B_obs| = %.4f, mean|dev| = %.4f\n', ...
        max(abs(bud_dev)), mean(abs(bud_dev)));
fprintf('  (if these are large, raise W.lam_B; near-zero means the soft constraint binds)\n');
...
% V17b: S is EXOGENOUS (C.S_exo); control u = [F_above;F_loans;F_guar;F_DI].
    ...
    xp(1) = C.mu_y + P.rho_y*y + P.alpha_S*S + P.alpha_above*a2 ...
          + P.alpha_below*st_used + P.alpha_AB*a2*st_used + P.alpha_DI*di1 ...
...
% optimum (constant in U).
    ...
    for q = 1:P.N
        ...
        bc = W.lam_B*(sum(u) - C.B_obs(q))^2;        % V17d: soft budget constraint
        J = J + W.beta^q * (sc + cc + bc);
    end
    dx = X(:,end) - W.xbar;
...
% B columns: 1 F_above | 2 F_loans | 3 F_guar | 4 F_DI.
    ...
    a2=x(6); st=x(8);
    flo=u(2); fgu=u(3);
    st_used = (1-P.decay_K)*st + P.c_lo*flo + P.c_gu*fgu;   % same as f_step
    A = zeros(P.n); B = zeros(P.n,P.m);
    A(1,1)=P.rho_y; A(1,4)=-P.beta_d;
    A(1,6)=P.alpha_above + P.alpha_AB*st_used;              % +d(alpha_AB*a2*st_used)/da2
    A(1,7)=P.alpha_DI + P.alpha_SDI*S;
    A(1,8)=(P.alpha_below + P.alpha_AB*a2)*(1-P.decay_K);   % +d(.)/dst via st_used
    B(1,2)=(P.alpha_below + P.alpha_AB*a2)*P.c_lo;          % +d(.)/dflo via st_used
    B(1,3)=(P.alpha_below + P.alpha_AB*a2)*P.c_gu;          % +d(.)/dfgu via st_used
...
% (a constant in the objective; zero gradient/Hessian).
    ...
    % V17d: soft budget constraint lam_B*(sum(u)-B_obs)^2.
    % Gradient: 2*lam_B*(sum(u)-B_obs) for every control (uniform vector).
    % Hessian:  2*lam_B for every (i,j) pair -> FULL matrix (sum couples all u),
    %           NOT diagonal. This off-diagonal coupling is what enforces
    %           reallocation: raising one control must be offset by lowering
    %           others to keep the sum near B_obs.
    sdev = sum(u) - C.B_obs(q);
    lu   = lu  + 2*dq*W.lam_B*sdev*ones(P.m,1);
    luu  = luu + 2*dq*W.lam_B*ones(P.m,P.m);
end
...
% NOTE (V17c): the above x below complementarity alpha_AB*a2*st_used also
% vanishes at the steady state (a2=0 AND st_used=0), and every one of its
% derivatives carries a2 or st_used as a factor -> zero at the origin. So the
% terminal LQ problem is UNCHANGED by alpha_AB; no entry added here.
